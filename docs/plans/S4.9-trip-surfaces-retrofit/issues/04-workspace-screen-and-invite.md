...
- [ ] The workspace screen renders chip, count, tabs; an archived trip explains itself above the tabs for owner and member (spec AC 10)
- [ ] The Chat tab greys with `comingSoon` + analytics and dead-clicks nowhere on web (spec AC 11)
- [ ] Details tab carries fields + Edit, archive link, leave/transfer; nothing from today's screen is orphaned (spec decision 8's redistribution)
- [ ] A full exact handle finds the traveler and invites them; the invitee's inbox shows it; a partial handle finds nothing; email invite still round-trips (spec AC 13 — client half)
- [ ] The network section renders its empty state — words, never rows — with its analytics call site (spec decision 11)
- [ ] Owner-only affordances (invite CTA per S1.2's owner-only issuance, archive link) render by role, from the roster the screen already fetches
