...
**9 · The emulator walk is done; only the deployed-dev probe remains.** Walked on Pixel_7 with `t1` as trip owner, JS from Metro (8082) against the local backend via `10.0.2.2` — no Gradle run, because this story is JS-only on the mobile side and a dev build takes its JS from Metro. Screenshotted end to end: Trips sections → create-entry → day tabs → Preview Itinerary → the honest banner with **0 / 0 / Est. Cost** → **Finish Planning** → workspace at *Planning Finished* → the gate dialog refusing publish and naming why. Server agreed: `state: upcoming, published: false, visibility: public`. The publish half of the walk is covered in the rebuilt preview container instead (`drive-publish.js` 36/36). **AC 15's post-deploy probe is the one thing still open** — the `ACTIVE → ONGOING` remap can only be proven on deployed `dev`, the sole database holding a non-DRAFT row, so it belongs to the promotion rather than the branch.

**10 · The code review found two real defects, both in what the founder would have seen first.** *Finish Planning was on the wrong screen* — docked on the day schedules where the spec (AC 2) puts it on the preview, and worse, a draft's preview still offered **Publish Itinerary**, a button the gate exists only to refuse. *The published stats card was never touched by the story*: it still read "Est. Total" with greyed dashes where decision 10 asks for "Est. Cost" and honest zeros — and `drive-publish.js` had been asserting the **old** label, so it passed 36/36 while the decision sat unimplemented. That is the S1.1 family again — a check whose outcome could not distinguish done from not-done. Both fixed and re-verified before the branch was pushed.
