...
**Three defects the code review found in this test, all fixed (2026-09-08).** Worth recording because each was the test asserting something weaker than its own name claimed — the failure mode a meta-test is least able to notice about itself.

- **"No guard *anywhere*" scanned five directories.** `allGuards()` walked only the modules on the guarded list, so `support/NewWorldBoundaryTest` — a real guard matching the same `*BoundaryTest.java` name — was never read, and an exemption added there would have stayed green. It now walks the whole test tree. Sabotage-checked: an exemption planted in `NewWorldBoundaryTest` now fails the rule naming that file, which the previous version could not see at all. `guardsOf` also gained recursion, so a guard in a sub-package counts.
- **The exemption pattern matched the word "exempt" in prose.** A guard is free to explain itself, and TW-1's planned wording (*"the legacy exemption, dissolving at CM-5"*) would have failed a guard that has no exemption in it. The pattern now matches the **construct** — `simpleName(`, `haveSimpleName(`, `belongToAnyOf(` — and the test asserts both directions: the construct fires, the prose does not.
- **One test was a tautology.** `theExemptionRuleWouldFireOnAReintroducedExemption` asserted the regex against three inline literals and never against a file, so it tested `Pattern.compile` rather than the tree. Rewritten as `theExemptionRuleMatchesTheCONSTRUCTAndNotTheProse`, which is what it is actually for; the vacuity guarding is done by `theScanReachesRealFilesRatherThanPassingVacuously`, which reads real files.
