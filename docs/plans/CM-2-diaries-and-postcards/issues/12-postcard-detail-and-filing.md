...
- *2026-09-07, at the code review:* AC 9 stays ticked, with one clause of it deferred — the visitor’s diary row **opens the diary** but does not land **at that day**; the same gap ticket 10 records and the epic map now carries. Everything else in that criterion is built.
- *2026-09-07, scope beyond AC 11:* the founder ruled Edit Postcard must also carry photos and, later, the location field, so it edits caption, photos and place. `PATCH /v1/postcards/{id}/place` is the additive route added for the last of those, with its own contract-doc row.
