...
- [ ] Diary detail (frame B) renders the cover header with the title, "destination • N days" and the date range over the gradient, the author row, the days in ordinal order with their postcards, an empty day's dashed placeholder, "+ Postcard" per day, the kebab, and the "Add a day" footer — every count, ordinal and date from the server's read
- [ ] The visitor's version (frame C) drops the kebab, "+ Postcard" and "Add a day", renders an empty day as a plain line, and makes the author row navigate to the profile
- [ ] A diary with no cover renders its first postcard photo in the header, from the server's fallback
- [ ] Opening a diary from a postcard's diary row lands scrolled to that day
- [ ] A stranger to a private author gets the profile-private refusal rendered as the app already renders it for a private profile; a follower gets the page
