...
- [x] ~~No mobile diff~~ — **corrected 2026-08-23.** The client composes nothing, which was the point of the AC, but it did need changing: `joinLinkOptions` held `staleTime: Infinity` (right at S4.28, wrong the moment this story gave the URL a version), so the copy button served a stale `?v=` until a page refresh. Fixed on `fix/S4.29-stale-invite-link`; spec decision 15 amended with the general lesson. Test-side changes also stand: `joinGate.test.ts` pins that the token parser ignores `?v=`, and `e2e/api/join-link.spec.ts` pins the suffix and the crawler split
