...
- *2026-08-08, two deviations surfaced at implementation, stated per the mock rule (decision 9's family — mock-set drift resolved by picking one side):*
  ...
  - **The card date renders at weight 600, where the mock draws 500.** ADR-016 loads four Inter weights (400/600/700/800) and no Medium, so 500 has no asset to render — 600 is the nearest loaded weight. Squarely inside decision 9's "normalize to Inter and the token palette", recorded here because a chosen-nearest should not pass as the mock's own value.

- *2026-08-08, **AC 8 closed — the three-rung smoke ran green**.* Backend ITs: not run and not needed — the story changed no backend file (wire changes: none). **Web preview container** (the true build path, `npm ci` + export inside the image, Caddy serving): the full create walk reached the overview with zero console errors, zero page errors and **zero anonymous `/v1` requests**; both greyed header icons and the greyed workspace door each raised their alert under CDP interception. **Emulator** (dev build, Metro over `10.0.2.2:8082`): the same walk, plus the two things only a device proves — the **native** coming-soon dialog on the greyed door, and **hardware back from the overview landing on Trips** with the new trip atop Drafts. An uploaded cover came back as `bearer GET /v1/media/…/thumb` and rendered on both rungs, exercising both `mediaSource` forks. Not exercised: the publication badge (needs a published trip and a second pool account; pinned by unit tests, and publish is untouched here).

- *2026-08-08, the cover-upload thread, opened and closed the same evening.* The founder found three things after the story was marked done: covers not appearing until a reload (a real stale-cache bug — the upload bypassed its mutation hook, so nothing invalidated; regression line 20), the upload happening at Create Trip rather than at pick, and an `Uploading…` status that was **false** on this screen (`CoverPicker`'s `busy` was bound to `create.isPending`, the *trip* request, so it announced an upload during the one phase where no bytes move). Fixed on this branch: the cache is told, navigation happens before the upload, the picked photo renders from the local file meanwhile, and the label now needs a separate `uploading` prop that only the edit screen passes. **The ordering itself was specced as S4.16 and declined by the founder the same day** — *"i think this is a better behavior than earlier"* — because what remained after these fixes is invisible, and every real fix costs a wire change plus, in two of three shapes, a scheduler and a quota. The four-call sequence and the server-enforced lease are recorded in the epic map so the client-side "fix" is not attempted again.

- *2026-08-08, review findings left standing on purpose (so they are not re-argued next session):*
  - **The thumbnail component is duplicated a fifth time** (`CoverThumb` on the card, `SummaryThumb` on the overview, alongside `Avatar`, `AvatarStack` and `PublishedItineraryView`). All five share one shape: `useMediaSource(thumbOf(...))`, a null-source fallback tile, the same four `<Image>` props. A shared `MediaThumb` is the right answer and is **not** this story's to write — the extraction touches four surfaces S4.15 does not own, and doing it here would hide a refactor inside a UI story. **Backlog line written to the epic map**, triggered by the workspace redesign or a sixth call site.
  - **Four button heights (52 · 53 · 46 · 46) rather than `controls.buttonHeight` (51).** Each is the mock's own value on its own frame, and decision 9 says the geometry copies verbatim; collapsing them to one token would be inventing a consistency the design does not have. Kept as mock fidelity, not drift.
  - **`tripCardAnatomy` holds two presentation formatters and is named for a shape.** Accurate enough — both functions answer "what does this card show" — and renaming buys nothing until a third card fact lands.

- *2026-08-08, verified against the backend rather than assumed:* decision 2's **"destination alone when Duration was skipped"** branch is reachable on real data — `CreateItineraryRequest.durationDaysOrZero()` returns 0 when the field is omitted and `DayService.seedDays` mints nothing at count 0, so a duration-skipped trip genuinely arrives with zero days. Checked because a branch that only exists in a unit fixture is a copy defect waiting for a founder to find it.

- *2026-08-09, **decision 8 is AMENDED by S4.19** ("the form simplifies its words, not its fields — the S4.13 field set is untouched"):* the workspace-smoothing grilling ruled the create and edit screens into **one shared form component with a mode contract**, and the create field set changes once as part of it — **Destinations become multi-entry** ("+ Add destination"), a founder amendment to this story's mock baseline, which draws a single Destination field. Everything else decision 8 locked stands: the chrome ("Plan a Trip" / "Create Trip"), the simplified placeholders, Duration minting days, the Trip Created overview. This comment exists so the next reader of decision 8 learns the field set moved rather than trusting it. Spec: `docs/plans/S4.19-workspace-editing-pass/spec.md`.
