...
**Corrected within the hour, founder: a centred popup, not a docked drawer.** *"I don't think that it should be a drawer, a popup modal perhaps?"* — and the correction is right for a reason worth recording, because it will recur: **a bottom sheet is the shape of an action, a centred dialog is the shape of a thing you look at.** The first build copied `FinalizeSheet` because it was the nearest prior art in the tree, which made it the nearest *mechanically* — scrim, modal, CTA pair — while being wrong *semantically*: Finalize asks you to commit to something, this only shows you a card. Nearest-prior-art is a good default for tokens and a bad one for form. The dialog now follows **`CircularCropper`** instead — `animationType="fade"`, centred backdrop, `maxWidth: 380`, all-round `workspaceRadii.card`, no grabber — which is this repo's existing centred-dialog pattern, so again nothing new was invented. `tabRouting` pins the centred shape (fade, `justifyContent: 'center'`, no grabber, no top-only radii) so it cannot quietly slide back down to the bottom edge.

**2026-08-10 — addendum 3, founder: closing must be instant, and the sluggishness was a real defect rather than a perception one.** *"When you close the modal on web, the modal removes the profile stub and then closes the window. It feels that it's not responsive."* Exactly right, and the sequence described **is** the bug: one piece of state, `traveler`, drove **both** `visible={traveler !== null}` **and** the card's own `{traveler !== null && …}`, so setting it to `null` blanked the card on the very frame the fade-out began — the traveler watched an empty white box fade for ~200ms. **Two systems disagreeing about one state, which is the S4.17 drag lesson in miniature**, and the fix is the same shape: remove the disagreement structurally rather than tune the timing. The card now reads a **retained** value (`stillShowing`, extracted pure and unit-tested) that holds the last traveler through teardown, so it can never blank early; and `animationType` is **`none`**, so the window itself leaves on the frame it is tapped. Founder chose instant in **both** directions over a fade-in/instant-out asymmetry.

**A trap that disappeared with the drawer, worth keeping anyway:** RN's `Modal` renders **outside** the `MobileFrame` view tree on web, so the first sheet ignored the 393px frame and spanned the whole browser window. A centred dialog with its own `maxWidth: 380` is narrower than the frame and so never shows it — but **`FinalizeSheet` still has the bug**, untouched here because it is pre-existing and not this addendum's to fix. Anyone finalizing a trip in the web preview sees a sheet the full width of their browser.
