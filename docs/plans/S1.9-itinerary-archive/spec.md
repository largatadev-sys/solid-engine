...
**2026-07-28 — implemented, tickets 01–05. Backend 302 ITs · mobile 587 tests · API smoke 46/46 · web 22/22 · device walked, all against the local full stack.**

1. **The module-placement question (ticket 02 §0) resolved to `membership`, and the cycle check nearly went the wrong way.** A first grep for `com.largata.membership` inside `invitation/` *hits* — which reads as "the reverse edge already exists, so this closes a cycle". The hit is `invitation/web/TripMembershipController`, a **controller** composing two services for URL cohesion (S1.5's deliberate choice). The service layers form a clean DAG. The discriminating check is *which layer holds the import*; recorded because the S1.5 comment warns about this trap from the other direction.
2. **The fence found a better shape than the spec described.** Decision 11 said "one checker, called explicitly at every write". True for the six governance endpoints — but all nine *plan* writes already funnel through `editLease.requireHeldBy` as their first line, so the fence went there: one call covering nine endpoints at a seam they are structurally required to pass, rather than nine copies a tenth method could forget. Ordered before the lock check, because on an archived trip nobody holds a lease and nobody may acquire one, so a lock-first order would answer *"«nobody» is editing"* to a question whose real answer is "this trip is frozen".
3. **Two carve-outs, both now tested rather than implied.** Invitation/offer **accept and decline** need no fence — archive voids every pending row transactionally, so the existing status checks refuse (predicted in ticket 03 §4, verified as `OFFER_NOT_FOUND`). **Releasing** the edit lock is deliberately unfenced while acquire and renew are: release gives up a right rather than claiming one, and fencing it would break its stated contract (a best-effort release on navigate-away must not fail) for no protective value. The second carve-out was undocumented until code review; it now carries its reasoning at the code and an assertion in `ArchiveWriteFenceIT`.
4. **`archived` ships as a boolean beside `state`, not a fourth state value** (decision 2 in code): the machines are orthogonal, a trip can be `completed` *and* archived, and under ADR-008 an `"archived"` state value would have been permanent.
5. **The list filter needed no bulk "which of these are archived" query.** Every row in a page carries the same value as the request that asked for it, so the flag *is* the parameter. A first draft of that query also had a real cardinality bug — selecting off `Membership` returns one row per member, so a 4-person trip appeared four times.
6. **Code review (two axes, parallel) found four defects; all four fixed before this note.** The serious one: **the Members screen was entirely archive-unaware** — three of the four frozen governance acts live there (remove, offer, withdraw) and all rendered as tappable guaranteed `TRIP_ARCHIVED`. Both axes found it independently. The device walk had missed it because the Members screen was only ever opened *as the member*, who sees just Leave — a correct result that masked the untested owner half. The gating now folds `archived` into `memberControls`, where `canLeave` sits beside it as the stated exception. **Verified by sabotage:** reverting the fix fails three web-driver assertions.
7. **A `canLeaveTrip` helper was written, then deleted.** It was a constant `true` whose stated job was to make the founder's rule findable — but it was imported nowhere, so it defended nothing. The rule moved to `memberControls`, beside the gate it is an exception to. A comment records the reasoning: the instinct (name the decision) was right, the file was wrong.
8. **Both the migration and the fence were sabotage-verified** (S1.1's rule). Inverting V13's `CASE` failed exactly the two mapping tests; short-circuiting `WriteFence` failed the enumeration test with `expected:<TRIP_ARCHIVED> but was:<EDIT_LOCKED>` — a useful shape, showing the two gates are layered rather than one masking the other.
9. **V13's backfill ran against real rows on the local stack** (up 12 hours with S1.7's data): `2 COMPLETED / 10 ACTIVE / 0 ARCHIVED`, each matching its itinerary — the rehearsal deployed `dev` needs, on the one class of migration every local surface is blind to.
10. **The preview screenshot caught what no assertion did:** every check was green while the badges read *Draft · private · archived* — the lifecycle badge is capitalised by `formatItineraryState` and mine was not. Fixed. This is why the repo pairs a screenshot with green tests.
11. **AC 15 (deployed-dev probe) remains open by design** — it runs after the promotion to `dev`, which is the owner's call. Ticket 05 §3 carries the recipe, including the rule that the SQL must name the `railway` database.
