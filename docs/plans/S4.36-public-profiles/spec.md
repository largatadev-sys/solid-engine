...
**Status:** built, web-proven — device walk deferred by founder call · **Epic:** E4 · **Depends on:** S4.21 (shipped — the layout the projection inherits), S4.3 (shipped — the Discover surface and its suggestions grammar), S4.22/ADR-025 (shipped — every diary entry is public), S4.23 (shipped — bylines carry the handle on the wire), S4.14 (shipped — the vanity number), S4.28 (shipped — member management re-homed to the Travelers tab, which this story therefore does **not** owe)
...
### 2026-08-26 — decision 6 amended: the four-cell row returns, with the follow counts unnumbered *(founder, at the build review)*

**The stats row is four cells again — Published · Postcards · Followers · Following** — restoring the own-profile rhythm the canvas's C3 had cut to two. Decision 6's *cells* are superseded; **its reasoning is not**, and the amendment is shaped by it:

- **Followers and Following render `—`, never a number, until story B fills them.** The stub metrics do not reach this surface. Decision 6's driver was never the cell count — it was that *no invented number about a real person reaches a stranger*, and that holds exactly as written. The founder's ground for restoring the slots: following ships next, so the layout should not move twice.
- **`Trips` stays off the row, and that half of decision 6 is untouched.** The owner's trip count includes private memberships, so it would tell a stranger how many private trips someone has. This is the leak the separate public stats read exists to make structurally impossible, and no rhythm argument reaches it.
