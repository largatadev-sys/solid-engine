...
**2026-08-26 — the second cell is Destinations, not Postcards** *(founder, same review; full reasoning in the spec's Comments)*. Distinct destinations on the stranger's surface. The founder asked for `Trips` from completed trips and accepted the privacy cost on the record; the shipped `Trips` count turned out to be membership-based (private trips and joined trips both), so it would not have meant what a visitor would read it as. Destinations carries the same influencer intent with every counted place visible to the reader.
