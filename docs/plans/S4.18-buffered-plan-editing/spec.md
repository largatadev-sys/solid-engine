...
- **`npm run smoke` run whole, and baselined against `dev` rather than trusted** *(regression-checklist line 17, which this exact question has now been asked over three times)*. API rung fully green — 9/9, 44/44 and S4.18's 15/15. Web: `drive-create-flow` **12/23** and `drive-publish` **24/12**, byte-identical to the recorded `dev` baseline; `drive-workspace` **38/13** against `dev`'s 37/13, compared **assertion-by-assertion from a worktree at `dev` serving a `dev`-built container** — zero regressions, one added assertion. **Both S4.18 walks are wired into the runner**, so this coverage survives the story rather than depending on anyone remembering it.
- **The smoke run caught three real stale assertions, which is why it was worth running.** `drive-workspace`'s drag checks verified a reorder by re-reading the API immediately after the drop — correct before S4.18, wrong after it, since the whole point is that the drop writes nothing. They now assert the staging: the **screen** reorders (`server Alpha, Bravo, Charlie | screen Bravo, Alpha, Charlie`) while the server does not, and `drive-buffered-plan.js` gained the leg that proves the other half — drag → server unchanged, no write → Save Changes → `Alpha, Bravo → Bravo, Alpha` in exactly one PUT. The founder's originally-reported op is now covered end to end.
