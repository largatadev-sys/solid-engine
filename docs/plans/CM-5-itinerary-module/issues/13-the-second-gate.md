...
`node mobile/scripts/walk-cm5.js` against a local stack built from this branch: **11 of 11**, t1 as author and t3 as stranger. It closes twelve of the thirteen steps whose answer is a status code, an id or a substring — **corrected at the final review, which caught the first draft claiming "every step" while the script never touched Discover, Home or the profile, the three surfaces tickets 05 to 07 rebuilt.** Discover, the showcase and Home are all covered now — **14 checks**, the last added when the founder asked why the gap was optional. Home needed a real multipart postcard with the committed JPEG fixture, and it pins ticket 06's central invariant: a feed card links to the **Itinerary** id, not the trip — including the three findings this story is most likely to regress on: archive fencing publish and unpublish, reopen refused on a published trip, and the trip's dates reaching the page nowhere.
