...
**2026-08-13 — and it must LOOK like the diary, which the first attempt did not** *(founder: "i think we changed the my diary page too much. what i wanted is just the trip post button to direct to that diary page, not reform it with these new cards from the home page")*. The public screen was built out of `FeedCard` because the data came from the feed's projection, so the destination arrived wearing the feed's author row, like counts, badge and card surface — a page the traveler had never seen, reached by a button promising one they had. **The author-facing screen was never touched** (only S4.21's "MY DIARY" overline came off, at the founder's earlier request), but that is not what the ruling is about: the complaint is that the *destination* was a redesign rather than the same page opened for a reader.

The fix is one component, not two implementations: `PostcardStreamEntry` holds the stream row — eyebrow over activity title, full-bleed paged photo, caption beneath — and both `TripDiaryScreen` and `PublicTripDiaryScreen` draw it. The public screen wears the diary's chrome and opens the same `PostcardPreview` on tap, with **Edit withheld** (`onEdit` optional, mirroring `onShareChange`) and its divider withheld with it. **Two screens rather than one**, deliberately: the reads differ in shape — the author's is guard-protected and takes no author param, the reader's is the public shared-only projection — and folding both into one component means conditionally-enabled hooks in a screen that is currently trivial. The shared component is what keeps them from drifting; the file split is only about where the data comes from.

**A consequence worth noting:** the public projection carries no `timeOfDay`, so a shared postcard's eyebrow reads `Day 3` where the author's reads `Day 3 • 07:40`. That is `snapshotEyebrow`'s existing null behaviour, not a special case — and adding the field to the projection for cosmetic parity was declined, since the eyebrow reads correctly without it.

**2026-08-13 — decision 1 is reversed before the merge: every diary entry is public, and the Share button goes back to being a share** *(founder: "i saw on the post cards that there is share to feed button. now, all diaries entries, if public are automatically in the homefeed. they default to public. revert that share button to what it was before, it was supposed to be a share diary entry you can send to someone. like literally sharing a postcard, it was never meant to be share to the homepage as diary entries are public by default"; and on being asked what gates visibility: "everything is diary entry is public for now, once a traveler adds a diary entry. the trip doesnt affect visibility. i dont know if there is already mechanism for private and public diaries but as far as i know it is not yet implemented, only for trips")*.

**The founder's read of the codebase was right about the world before this story and one story out of date about this branch:** no per-entry visibility mechanism existed until S4.22 built one, which is precisely why the reversal is cheap now and would not have been later. `visibility` remains a *trip* axis (ADR-019) and does not gate entries.

**What that made of the story's own work.** Posting is publishing: the entity stamps `sharedAt` in its constructor so no call path can produce a private entry, and **V30 backfills every existing row from `created_at`** — a row left NULL would be permanently invisible with no UI able to surface it, and `now()` would file last month's trip at the top of the feed claiming a moment that never happened. Deleted rather than parked: `POST`/`DELETE /diary/entries/{id}/share`, the request's `shareToFeed` field, the entity's mutators, the composer toggle, the mutation, both repository methods. **Cut now, deliberately, because ADR-008 freezes /v1 at the merge** — an endpoint left in could be added back later but never removed. The preview's Share is `comingSoon('share')` again, which is what `dev` already had and what "revert that share button" names.

**Two defects the tests found rather than the author.** `post()` still emitted `diary_entry_shared` beside `diary_entry_created`, which would have double-counted the funnel on every post; and `DIARY_PRIVACY_NOTE` still read *"Only you can see your diary"* — the exact opposite of true. S3.1 pinned that string in a test so it could not drift silently, and the pin did its job a story later. It now reads *"Your postcards go to the Home feed, where any Largata traveler can see them."* — shipped for veto on sight, and stated **before** the first postcard rather than after.

**The cost, on the record:** a traveler's only retraction is delete. There is no post-but-keep-this-one-back. The backfill is irreversible and was taken on the standing ground that the installed base is the founders' own. `shared_at` keeps its nullability because the mechanism is parked, not deleted — the friend graph re-enters through that column.

**2026-08-13 — archiving a trip withdraws its postcards from the feed** *(founder ruling, on being shown 117 shared postcards belonging to archived trips still public on Home)*. Found while seeding test data, not by a test: the seeder archives its previous trips on each re-run, and their postcards kept accumulating on the feed in plain sight.

**The defect was an inconsistency, not a missing feature.** `navigableTripOf` already withheld the trip *link* for an archived trip while the *card* stayed — the product answered the same question two ways. Deciding either way would have been coherent; being half-aware was not.

**Why withdraw rather than make the link stay.** Decision 1's reversal deleted unshare, so delete — per-entry and permanent — became the only exit. Archive is the gesture a traveler already reaches for to put a whole trip away, it is **reversible**, and there is no trip-delete endpoint anywhere in the API. The counter-argument (archive is a workspace concern; a postcard is the author's deliberate broadcast) was heard and declined: a bulk, reversible retraction is worth more to a traveler than module purity.

**One filter, both surfaces.** `PostcardFeedService.project` excludes entries whose trip is archived, so the feed drops the card *and* the per-trip public diary 404s — they cannot disagree, because the diary reads through the same projection. The lookup is batched through `WorkspaceService.archivedAmong` rather than asked per entry (a page of 20 costs one query, not twenty) and stays on the workspace module's service rather than joining its tables (ADR-002).

**Consequence worth knowing:** a feed page can now return fewer cards than its limit, because the filter runs after the page is read. Pagination is unaffected — the cursor comes from the last row *read*, not the last card *kept*. Four ITs pin the rule and were sabotage-verified: replacing the filter with `entry -> true` fails all four, naming the feed, the round trip through unarchive, the 404, and the per-trip scoping.
