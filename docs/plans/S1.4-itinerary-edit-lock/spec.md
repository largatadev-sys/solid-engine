...
**2026-07-24 — implementation notes (tickets 01–03).**

1. **The "remove plan writes from the offline queue" AC was a no-op, and that is the honest finding.** ADR-001's queued writes are a *declared* standing behaviour, but **no write-queue exists yet** — `queryClient.ts` is an in-memory read-cache only (its own comment: "Offline-from-cold-start is E3's story… arrives as a persister on this same client"). So there was no queue to exclude plan writes from: an offline plan write already throws `ApiError.offline()` immediately (never defers), which is exactly the behaviour ADR-014 requires. The mobile side instead makes the *edit-entry* honest — `useEditLock.acquire()` fails offline and the surface shows the offline message via the platform-forked `editLockedAlert`. When E3 builds the real write-queue, it must exclude plan writes by construction; that constraint is recorded in ADR-014's consequences, and there is nothing to gut today.

2. **TTL / renew cadence settled (spec §lease):** TTL **3 minutes** (`largata.edit-lock.ttl`, default `PT3M`), client renew every **60 seconds** (`EDIT_LOCK_RENEW_MS`). The backend reads time from an injected `Clock` so the expiry/renewal ITs advance a `MutableClock` rather than sleeping.

3. **The conflict envelope:** `409 EDIT_LOCKED`, message = the holder's display name + " is editing this itinerary right now." (resolved via `TravelerService.summariesByIds`, the S1.2 cross-module lookup). `edit_lock_denied` is emitted **immediately**, not after-commit — a denied write rolls its transaction back, so an after-commit emit would be discarded and the contention funnel would never count the collisions it exists to measure. `edit_lock_acquired` / `edit_lock_expired_takeover` stay after-commit (they only mean something if the acquire committed).

4. **The AC-7 reversal, as executed:** the S1.3 `ActivityContractIT.twoMembersEditingTheSameActivitySequentiallyLastWriteWins` test is **deleted** (a marker comment left in its place naming the S1.4 reversal), and `EditLockEnforcementIT` pins the replacement — every one of the 9 plan-write families refused `409 EDIT_LOCKED` without the lease, and succeeding with it. The S1.3 storage/contract tests now acquire the lease in their fixtures (a `lock(...)`/`acquire(...)` helper), a one-line-per-fixture change; the guard-masking tests deliberately do **not** lock, since a non-member is 404-masked before the lease is consulted (their assertions are unchanged, proving the ordering).

5. **Mobile lock UX (ticket 03):** `useEditLock` acquires on entering an edit surface (`edit.tsx` fields form, `days.tsx` plan hub), renews on the 60s heartbeat, releases on save/unmount. A denied acquire shows the modal and routes back to the read-only plan; a mid-session loss surfaces through each screen's existing mutation-error path (every write then 409s). The modal is platform-forked (`editLockedAlert.native/.web`) with shared wording in `editLockedMessage` — the S1.3 `Alert.alert`-is-a-no-op-on-web discipline. **Not yet wired:** activity sub-screen (`activity.tsx`) relies on `days.tsx` holding the lease beneath it in the nav stack; the ticket-04 device/preview walk verifies that holds in practice.
