...
**2026-08-21, two more founder rulings on the running build — the chat head shows a real photo, and it bottom-aligns.** *(1) The avatar renders the traveler's profile photo.* C2 says *"28px **initials** avatar (stable per-member tint from the profile palette)"* and this spec's Mechanics listed exactly three author fields — so the wire never carried an avatar and the surface could not have shown one. The founder read initials-only as broken, and they are right that it is inconsistent: every sibling surface (Travelers tab, profile, feed) renders the real photo. `ChatAuthorResponse` gains `avatarUrl` (additive, ADR-008, no waiver) and the bubble uses **`MediaThumb`** — the same component the Travelers tab uses, which matters because our media endpoint requires a bearer token and a bare `<Image>` URL silently fails (the S3.3 trap). The tinted initials survive as the **fallback**, so the canvas's palette rule still governs whenever a traveler has no photo. *(2) The avatar bottom-aligns with the last bubble of its group,* Messenger-style, where the canvas prose says *"top-aligned"*. **The founder's argument is better than the prose:** the canvas's own geometry puts the **6px corner on the LAST bubble of a group**, and that corner is a tail — a tail only makes sense pointing at the avatar. So the artboards and the prose disagreed with each other, and the geometry wins. The handle stays where it was, at the top of the group.

**2026-08-21, founder ruling — M1 rises over 250ms, and it was being cut short after the first message.** *(1) The duration.* The canvas sets M1 at *"150ms ease-out"* from the shared vocabulary; the founder ruled 250ms on the running build. Recorded as the sixth deviation. The other 150ms values are untouched — M2's send crossfade, M4's failure and recovery fades, and the counter — so the shared *state-change* value still means 150 and only the message entrance departs from it. *(2) The defect that prompted the look, which was real.* `MessageEntrance` derived `settled` from its `animate` prop and listed it in the effect's dependencies, while `ThreadRowView` computes `animate` as `!entered.has(key)` and marks the key **during render**. So the second render of a row — and sending always causes one, as the draft clears, the pending entry settles and the socket write lands — flipped `animate` to false, re-ran the effect, and called `entrance.setValue(1)`, **killing the animation mid-flight**. The first message survived because nothing re-rendered inside its window; every message after it was cut, which is exactly the founder's report that *"the motion disappears on recurring messages"*. The entrance is now decided **once at mount** (`arrivedFresh`), the effect no longer depends on the prop, and the timing is stopped on unmount rather than snapped to its end. **Verified by sampling the DOM every 16ms across three consecutive sends** — each shows a clean ease-out ramp (≈2 → 92% opacity) with a transformed node present for the 8px rise; the first probe attempt reported a jump-cut for all three and was itself wrong, guessing at which node carried the opacity, which is why the sampler was widened to look for *any* fractional opacity in the subtree.
