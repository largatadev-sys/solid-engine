...
Decision 17 and C7 already specified it — *"everything else exits to the Trips tab (signed-in) or closes (web)"* — and nothing rendered it, so a traveler who asked to join was stranded on the pending card with only the browser's back button. A quiet "Back to my trips" now sits below the card (not inside it, so the postcard still speaks once), rendered whenever the viewer is signed in. Deliberately **not** a trip preview: the postcard never shows roster names or plan content, and opening one would hand plan content to someone the owner has not approved.

**Amended 2026-08-24 (founder walk): members get the exit too — the original rule excluded them and was wrong.** It read *"signed in and not already a member"*, on the reasoning that `Open trip workspace` is already an exit so a second one is clutter. **The two exits do not go to the same place.** The CTA opens *that trip*; a member who taps their own invite and does not want to open it had nowhere else to go — and the join route is registered `headerShown: false`, so on a device there is no back chevron and the browser-back escape does not exist. One tap, and it was the one they might not want. The rule is now simply **signed in → an exit to Trips**, which is what decision 17 said in the first place; the exclusion had quietly contradicted the sentence it was implementing. Signed-out visitors still get none, and that stays deliberate — they have no Trips, and the CTA is the intended funnel.
