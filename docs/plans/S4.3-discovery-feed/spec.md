...
**Glossary entries to land with this story** (held from mid-grilling per the owner's interrupt; apply to the domain model with owner review): **Trending Destinations** (derived projection over published itineraries' destinations, publish-counted, never curated, never an entity) · **Recommended (Discover)** (the curated rail whose long-term meaning is promoted placement; v1 editorial default; deliberately not interest-matched) · **Trip Ideas** (*reserved term*, backlog: the saved-itineraries collection the bookmark stubs point toward).
