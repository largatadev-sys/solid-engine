...
**Status:** drafted 2026-09-07 — grilled that day at TW-1's grilling (record `docs/plans/TW-1-trip-workspace-module/grilling.md`, round 1 Q1–Q4, Q7, Q13 and round 2 Q5, Q8, Q9); awaiting the owner's review and `/to-tickets`; **ships first in the content-remodel arc and waits on nothing** · **Epic:** none (the content-remodel arc, after CM-2) · **Depends on:** CM-1 (merged — the `/v1/trips` root, `DELETE /v1/trips/{id}`, the itinerary module's publish route), CM-2 (merged — the content readers already cut over) · **Branch:** `feature/CM-3-trip-grammar`.
