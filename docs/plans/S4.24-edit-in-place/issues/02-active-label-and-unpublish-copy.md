...
**What to build:** The trip being lived reads **"Active"** everywhere state renders — the viewer's badge and the trips-list rows — while the wire, the TS union and storage keep `ongoing` (label only, the `upcoming`/"Ready" pattern; ADR-027 priced the wire rename and declined it). The unpublish confirm dialog stops claiming the trip "goes back to being a draft" — false since the axes split, only `published` flips — and says what unpublish does: the public page comes down and editing thaws.
