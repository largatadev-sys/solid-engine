...
- **Pool:** one row per unclaimed number per month, generated as a single batch (0000–9999, shuffled) the first time the month's pool is needed — no scheduler. A claim pops one row transactionally with skip-locked semantics inside the provisioning transaction, so a rolled-back provisioning returns its number by transaction atomicity, and concurrent sign-ups never collide. A depleted month continues past 9999; width is a display concern, not an allocation one. **There is exactly one allocation path:** the backfill migration generates month `01`'s pool itself and claims from it for every existing traveler, so a lazily generated pool and backfilled numbers can never collide — lazy generation only ever creates a pool that does not yet exist.
- **Allocation timing:** at traveler provisioning, in the same transaction as the insert. Existing travelers are covered by the backfill (cohort derived from each row's creation timestamp — every pre-launch row lands in `01`; pool numbers claimed from the month-`01` pool the migration generates, through the same claim mechanism provisioning uses, so no number can ever be handed out twice).
...
- **Handle-save fix, universal and founder-blind** (the no-inline-tier-check rule stays intact — nothing learns what a founder is): three pieces. The backend treats a submitted handle equal to the traveler's currently stored handle as a no-op, skipping shape validation for the unchanged value, so any stored handle survives profile saves from any client. The profile-edit client sends the handle only when it actually changed. And the profile-edit submit gate treats a handle equal to the traveler's stored one as submittable without shape checks — without this third piece the Save button locks on a prefilled 2-character handle before any request is ever built, and the story fails its own acceptance. The backend no-op alone protects the data; the two client pieces make the screen usable.
...
- **The backfill gets a migration-stepping IT** (own container — never the shared singleton —, Flyway to the version before, legacy travelers seeded by raw SQL *including rows bearing the founder-UUID literals*, then stepped to head): cohorts derive from creation timestamps, founders get `(0, 0)`, non-founders get distinct pool numbers, and every backfilled number is absent from the remaining month-`01` pool (the collision the single-allocation-path decision exists to prevent). Sabotage-verified, using the resource-recompile invocation the repo has already recorded — without this IT the founder UPDATE matches zero rows on every test surface and the backfill "passes" as a no-op, the exact recorded data-migration trap.
...
- **Mobile unit tests:** the profile screen renders the served string verbatim; a pinning test on the profile save — an unchanged handle is not submitted, a changed one is; and the submit gate stays enabled when the prefilled handle equals the stored one, including a 2-character stored value.
...
**2026-08-08 — three founder rulings taken during implementation.** Recorded here rather than edited into the body above, because a spec is immutable point-in-time intent (`docs/agents/issue-tracker.md`).

1. **The handle minimum drops from 3 to 2 for everyone**, superseding this spec's *"`Handle` validation is untouched — minimum 3 for everyone, forever"* and its matching out-of-scope line. Founder reasoning, verbatim: *"we are just the ones onboarded with the app."* The consequence accepted knowingly: 2-character handles become first-come-first-served, and there are only ~1,296 of them. **The raise back to 3 before the first alpha tester is an epic-map backlog line with its own trigger**, and it is safe precisely because this story shipped the unchanged-handle no-op — a minimum is a claim-time rule, so a stored handle below a later minimum survives. The candidate-capability note is unchanged and still the durable answer: at register #14 the minimum returns to 3 and founders hold theirs through `can(traveler, capability)`. **What did not change:** no founder-conditional validation anywhere, so the no-inline-tier-check rule holds.
2. **The founder grant is deferred to a follow-up migration.** The UUIDs need a query against the deployed rung and the handle picks were unsettled; the founder chose to ship rather than hold the story. V25 carries an inert empty list, `VanityBackfillIT` pins that it *is* empty, and founders hold ordinary beta numbers meanwhile. **The follow-up needs its own migration-stepping test** — nothing here discharges that obligation. Ruling 1 shrinks its scope: it carries the `(0, 0)` grant alone, since founders can now claim short handles themselves.
3. **The vanity number replaces the raw UUID on the profile** rather than rendering beside it, resolving the spec's *"in the slot that currently shows the raw id"* in favour of the problem statement's framing (*"machine identity, meaningless to a human"*). The id remains in every `/v1/me` response and the database.

**2026-08-08 — one testing decision adjusted.** This spec asked for *"the profile screen renders the served string verbatim"* as a mobile unit test. The repo has no React render-testing library and adding one is a dependency decision outside this story, so both shapes are covered where the decision actually lives: `VanityNumberTest` pins the founder `"0"`, the padding and the overflow widening; `VanityNumberAllocationIT` asserts both shapes over HTTP. The screen holds no logic to test. The change-only handle payload was extracted into a pure `profileSaveFor` so it *could* be pinned, matching how this repo tests logic.
