...
**Status: BUILT 2026-09-08 — the record of what shipped is the ten tickets' Comments in `issues/`, and ticket 10 is the gate. PULLED 2026-09-07 — grilled that day (grill-with-docs, three rounds; the record is `grilling.md` beside this file); `spec.md` is the story's intent and this file is its background.** The grilling amended what is below in six places, each recorded in the spec: the trip half's Java types take Trip names in a separate commit after the relocation; the content half of the old package stays under the legacy exemption recorded here (the planning pass had set it aside for an api; the grilling restored it once the founder's model made that api throwaway); no schema move and no FK drops; the topic listener moves to `ws`; fork moves in whole; the move is sliced behind branch-local guard windows. The wire half — `/v1/trips/**` for the whole workspace — is **CM-3**, a separate story that ships first and reverses this design's "paths unchanged" on the founder's ruling that **"a trip is the trip itself"** (ADR-037). The conventions this design proposed are settled for every module at ADR-038. Where this file and the spec differ, the spec wins. **Trigger was: founder pull.**
