...
- [x] The trip guard is in its final form with the legacy exemption as its only named consumer, and the meta-test passes with the trip module under the rule and the old package alone on the dismantled list
- [x] The assertion-diff script's output is in the pull request, with the count of pre-existing assertions changed stated as zero, and the backend unit and integration counts read from the CI log
- [x] Hibernate's schema validation passed at the first boot and is cited; no migration shipped, no path changed, no client file changed — checked, not assumed
- [ ] The LAN walk is recorded with which tag played which role, and the backend log lines it produced name the trip module — **OWED, and deliberately not ticked.** The log half is proven (seven loggers, ids only, recorded below); the LAN rung was never stood up, so the criterion as written is unmet. This is CM-5's trigger
- [x] The epic map's line records built with every divergence from the spec; the design record's status line is closed; the tracker row carries status and spec link only, in the last commit
- [x] The pull request is open against the shared branch with no agent attribution in its body, and has not been merged
...
**Corrected at review, and the correction matters more than the run.** What follows first claimed a walk. It was not one. The 86 specs are exactly `e2e/api/` — which `ci.yml` already runs on **every** PR, alongside the `e2e/web/` project this run did not touch at all. So the *specs* proved nothing CI had not already proven twice. **What was genuinely new is the log reading**, and that is the part worth keeping. **No LAN rung was stood up**: not the phone, not the emulator, not `10.0.2.2`, not the `$LAN:8081` preview. Ticket 10 asks for a walk *"closed by hand on the LAN rung… from two phones"*, and none of that happened.
...
**What did NOT run, in full.** Not the LAN rung on a phone, not the emulator (blocked by the recorded Gradle fault, four stories now), not the `e2e/web/` Playwright project, not a hand-driven pass of any kind. **Everything the device rung exists to prove therefore stays unproven: native 1:1 gesture handling, keyboard avoidance, hardware back, Reduce Motion, safe-area insets — and the whole web-lane twin of the specs above.** TW-1 changed no client file, which is why the risk is low; low is not zero, and the criterion is **left unticked** rather than argued down to what an automated suite happens to cover. The founder's device walk is still owed, and it is still CM-5's trigger.

**And the three follow-ups the founder queued, all done here.** (1) The duplicate owner refusal is consolidated: three variants inside the trip module — `trip/exception/NotTheTripOwnerException`, `trip/trip/exception/NotTripOwnerException`, and `MembershipExceptions.NotTripOwnerException` — became **one**, the published one at the module root, carrying all five named factories. All three were `ForbiddenException("NOT_PERMITTED", …)`, so the wire is untouched; `join` keeps its own, being a different module. (2) The event-loss gap is an epic-map backlog line, **ranked**: `MembershipEnded` is the one that matters, because a departed member keeps a live subscription; `TripArchived` is not exposed at all, being BEFORE_COMMIT. (3) All **25** pre-existing unused imports are gone — a sweep now reports zero across 801 files. **This overturns a ruling recorded two comments above**, where the same sweep found them and left them alone as *"unrelated churn to a PR whose whole claim is that nothing observable moved."* The founder asked for them removed; the earlier reasoning was not wrong, it was outranked. Saying so matters more than the imports do — a record that quietly contradicts itself is worse than either decision.

**Two things the review changed, and one it did not.** `readOnly = true` beside `MANDATORY` on the two `*Rows` reads was **decoration**: a method joining an existing writable transaction ignores it, and 06b §11 says `readOnly` is a real hint rather than an annotation to sprinkle. Dropped. The **five named factories sit beside a public `(String)` constructor**, which the review fairly called half-refactored — six sites still mint free text. It stays, and the line is deliberate rather than accidental: the factories name **trip's own acts** (start, complete, remove a member, archive, offer, revoke), while the constructor is the seam for **another module stating its own condition** — publication's publish and unpublish, the old package's preview. Naming all eleven would put publication's vocabulary inside trip's published exception, which is worse coupling than the duplication it removes. What the review is right about is that the line has to be *stated* to be a line at all.
