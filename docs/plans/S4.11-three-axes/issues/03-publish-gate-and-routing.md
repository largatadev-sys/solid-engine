...
- [ ] Publishing from draft or active shows the precondition in words, naming *Mark complete* as the way through — never a bare disabled button
- [ ] The 409 `ITINERARY_NOT_COMPLETE` from the API surfaces as that same message, so a race gives the same wording as the pre-check
- [ ] Publishing a completed trip succeeds and defaults to Public; the success arc and published page are unchanged from S4.1
- [ ] The audience toggle works before publishing and after, and never changes `published` or `state`
- [ ] Drafts and active trips route to the workspace; published trips route to the itinerary overview
- [ ] A published trip's editor shows the frozen notice rather than the form (S4.1 behaviour, re-verified against `published`)
- [ ] Walked end to end on the device: create → start → complete → publish → freeze → unpublish → thaw
