...
**2026-08-25, reconciliation with S4.34 as built (pre-implementation, no re-grill).** S4.34 closed today — web rung green, device rung deferred **into this story** by owner call — so the dependency line above is satisfied, and ticket 07's cross-story blocker (S4.34 ticket 02) is discharged. Its implementation produced facts this spec was written a day too early to know. **None touches a locked decision**; each is recorded in the ticket that will trip over it, per the immutable-intent rule:

- **Ticket 08** inherits S4.34's deferred device scope — its AC 3, the device half of its AC 6, the silent-revalidation fix's device confirmation, and the travelling S4.34 gate-record Artifact — plus the workstation's Gradle configuration-time blocker, proven not-branch-caused. An owner decision (Home's anywhere-in-stack retap) waits on the same pass.
- **Ticket 06** absorbs the Travelers-tab focus revalidation that S4.34's review reverted as out-of-scope there (epic-map line, trigger *"S4.35, by name"*) — without it the reconnect contract never reconciles that tab until remount — and writes its absence proof on `trackApiTraffic`, the in-flight-aware settle S4.34's flake diagnosis had to build.
