...
**2026-08-25, owner call — the device rung is DEFERRED, and the story ships web-proven.** The blocker is a missing gitignored `mobile/google-services.json`, which needs a Firebase-console download only the owner can make; rather than hold a finished story open on it, the owner deferred the walk and authorised the promotion.

**What that leaves unproven, restated here so it is one list rather than scattered:**

- **This story's AC 12** — background the app, a co-member edits and approves, foreground, Trips is correct. WS-1's AC 10, now deferred a **fourth** time (WS-1 → S4.10 → S4.35 ticket 04 → here).
- **S4.34's AC 3** (`AppState` foregrounding) and **its AC 6 device half** (retap with real touch), which rode here by the 2026-08-25 owner call.
- **The device confirmation of S4.34's silent-revalidation fix** — react-native-web's `RefreshControl` is inert, so no browser walk can ever see whether a focus revalidation spins the pull control.
- **Every motion added at the owner's request.** Verified on the web preview only, and **motion is the most platform-divergent surface in React Native**: `Animated` maps to CSS transitions on web and to the driver on native, and the continuous `LivePulse` loop is the one most likely to differ in feel and in battery cost. Nobody should call these done until they have been seen on a device.
- **The owner decision on Home's anywhere-in-stack retap** (epic-map line), which only becomes observable on that pass.

**The gate-record Artifact remains unpublished, deliberately** — its own framing requires every claim to come from an observed run, and the device half produced none. It should be written when the device rung closes, carrying this list at the same visual weight as the greens, and S4.34's own gate record travels with it.

**2026-08-25 — the single-replica pin box stays UNCHECKED, and here is exactly why.** The box asks for it *"in place as configuration, with the constraint stated where the deployment lives rather than remembered"*. The constraint **is** stated — ADR-030's amendment ends *"the service is pinned to one replica, and the broker trigger is the instance count changing"* — but nothing in this repo enforces it: there is no `railway.json`, and the replica count lives in Railway's dashboard, which an agent cannot set. So today the pin is precisely what the box was written to prevent: **remembered, not configured.**

**Why it was not fixed at the gate rather than recorded.** Committing a first-ever `railway.json` minutes before a promotion would put an unexercised deploy-config file on the path to prod, where its interaction with the existing dashboard settings is unknown — a deployment change dressed as a checkbox tick. The safer order is: the owner confirms the current replica count, then a `railway.json` pinning it lands as its own small change with a deploy behind it.

**The consequence if it drifts, stated so nobody has to re-derive it:** the ticket store is in-memory and the fan-out in-process (WS-1 decision 3), so a second replica means tickets minted on A are unredeemable on B and broadcasts on A never reach B — presenting as **intermittently dead sockets with no failing test and no log line naming it**. That is the exact failure mode this pin exists to prevent, and it is invisible to CI.

**2026-08-25, code review round 3 — a real product bug found by a test I was writing to close an overstated claim.**

**The bug: `membership.granted` never cleared the approved traveler's pending row.** Ticket 04's box claims *"One event, two parts of one screen: approving a join request makes the trip appear in the list **and** clears the pending row from the inbox header"*, and round 2's review noticed the walk asserted only the first half. Writing the missing assertion is what exposed it: the pending row is fed by `useMyJoinRequests()` → `joinKeys.mine()`, while `refetchTripsAndInbox` invalidated only `itineraryKeys.lists()` and `invitationKeys.inbox()`. The row therefore sat there until something else refetched. **The claim was half-false and the checkbox had been ticked for it.** Fixed by invalidating `joinKeys.mine()` too, guarded by a Jest test asserting all three keys, and the walk now proves both halves against a count taken before approval.

**Also fixed from round 3:**

- **`terracotta220` violated the palette's own alpha convention** — a hard standards violation. The file names alpha as a **0–255 byte** (`terracotta016` = 16/255 = 0.0627451; `navy140` = 140/255 = 0.5490196), so `terracotta220` at 0.22 promised **four times** the opacity it had. Renamed `terracotta056` (56/255 = 0.2196078). `layering.test.ts` cannot catch this — it only bans literals outside `theme/`.
- **`LiveAdvisory`'s first entrance could not collapse** — found independently by *both* review axes, which is strong signal. `naturalHeight` is `null` until `onLayout`, so the row painted at full height and then snapped to `progress × naturalHeight` mid-animation: a pop, then a collapse, then a grow. It now holds height `0` while measuring — the inner view still lays out inside `overflow: hidden`, so the measurement is unaffected — unless the component mounted already-shown, where an unconstrained height is correct and no animation runs.
- **`Math.round` → `Math.ceil` on the measured height.** On Android's fractional densities, rounding down clips a descender.
- **`LiveValue` no longer changes the subline's box.** It had `alignSelf: 'flex-start'` plus a negative horizontal margin, which turned a stretched row into a content-sized one bleeding into the gutter — the S3.1 family (*truncates on the phone frame and nowhere else*), invisible in a wide preview viewport. The wrapper is now layout-neutral and the wash spans the row.
