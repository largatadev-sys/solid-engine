...
9. **The trip workspace gains an "Edit itinerary" cogwheel, opening the day editor.** Founder's call, and it completes the draft landing: a draft row opens the workspace, and the workspace is where you decide what to do next — publish, invite, or go build the plan. Shown only while the plan is editable (`canEditPlan`, i.e. not archived); when ADR-018 lands that becomes "only while draft", which is the one line that will need changing.
   **The two-cogwheels wart is resolved — founder took both halves of the fix.** The workspace affordance is a **pencil** (edit, not settings — the conventional reading), and the **day editor's gear is removed** outright, its back button carrying the return trip. One icon, one meaning, one screen. This retires S4.9's "the settings gear replaces the 'Details' text on the editor".

10. **The Details tab loses two controls.** *"remove the members & ownership transfer, this feature should be moved on when you click the members, and also remove the archive trip link for now."*
   - **Members & ownership transfer** — the button was a second door to a screen the roster row above it already opens (avatar stack + "N members"). Two doors to one room; the row wins because it is where you are already looking when you think about who is on the trip. The members screen itself, ownership transfer included, is untouched.
   - **Archive trip** — the quiet link is gone, so **archiving is now unreachable from the UI entirely**. The consequence to know: the archived banner and its Unarchive control can therefore only appear on a trip archived before this change or through the API. The backend, its fence, its ITs and the whole audience ladder are untouched and still tested — this is a UI removal, and `POST /archive` still works.
   - The dead client chain went with it (`ArchiveTripLink`, `useArchiveTrip`, `itineraryRepository.archiveTrip`), following the E1 gate's precedent: client code carries no additivity obligation, dead code is P9's speculative generality, and git remembers. `useUnarchiveTrip` stays — the banner still needs it.

11. **Trips loses the All chip and opens on Published; the itinerary overview gets a back to Trips.** Both founder calls, both small. The overview's back is a **reversal of implementation note 5's "no back control at all"** — the mock draws no header bar there, but a screen reached from a list needs a way back to it, and `useSafeBack('/')` gives it one that works from a deep link too. `seed-demo.js` joins the harness so the screens have honest content to render: four trips (two published, one private with a collaborator, one empty "someday"), `t1` owning all of them, `t2` collaborating on two, `t3` joining nothing so the masking cases stay testable.

12. **BLOCKED, and worth stating precisely: "private itineraries should have the itinerary overview" cannot be built on the current model.** Under what is shipped, `visibility=private` means *not published* — it is the only non-published value there is. So routing private → overview would send **every unpublished trip** to the overview and leave the workspace unreachable, contradicting *"draft workspace will be the trip-workspace screen"* from the same conversation. The two instructions are only compatible once **ADR-018's three-valued status exists**, which is exactly the distinction it introduces: `draft` (workspace) versus `private` (published, restricted → overview). It also needs the projection endpoint to admit collaborators on a private itinerary, which is a real backend change, not a routing tweak. **The naming blocker is resolved:** publication status ships as a new field **`status`** (`draft | private | public`); the dormant lifecycle keeps `state` and stays invisible, so the two "draft"s never meet.

13. **ADR-018 is built — 2026-08-03.** The founder confirmed the model and added the clause that settled its last question: *"a private or public can't be draft so it shouldn't show on draft pill."* Mutually exclusive, therefore one three-valued field. What shipped:
   - **`status` replaces `visibility`** — `draft` (owner + collaborators, **editable**) · `private` (published, owner + collaborators, **frozen**) · `public` (published, everyone onboarded, **frozen**, and the default). V20 backfills `PUBLISHED → PUBLIC` and `PRIVATE → DRAFT`, then drops the old column: a rename-and-widen, not a deletion, and `ItineraryStatusBackfillIT` steps the migration on its own container to prove both arms, because a data migration is invisible to every other test surface this repo owns.
   - **The freeze lands at `WriteFence.requireEditable`**, which the three plan-write services call — and, crucially, the **edit lease** calls, so every header/day/activity edit is covered by one chokepoint rather than a remembered check at each door. `requireWritable` (archive only) stays on the membership and publish/unpublish paths, so a published trip can still be unpublished, invited to, and transferred. A distinct `ITINERARY_PUBLISHED` 409 says *why*, and the mobile wording turns it into "unpublish to edit — it goes back to a draft".
   - **The projection's admission rule is now three-way**: public admits any authenticated traveler, private admits collaborators, draft admits nobody (the owner previews through the other door). That needed `AuthorizationGuard.membershipOf` — the guard gains an optional resolve so the public read can ask "is this traveler a collaborator?" without throwing, and `requireMember` is re-expressed in terms of it, keeping one resolution point.
   - **Mobile**: three chips with no overlap, draft → workspace and private/public → overview, an audience picker on the preview (Public default, one tap to Private), an audience toggle in Details, and the pencil hidden unless the trip is a draft.
   - **Not done, and worth knowing:** the `draft → active → completed` lifecycle is still on the wire and still dormant. It now shares a word with the publication status but nothing else; `state` is invisible in the UI and `status` is the only one anything reads.

8. **The model itself was re-drawn — ADR-018, proposed 2026-08-01.** *(Superseded by note 13: built 2026-08-03.)* Later the same day the founder replaced the binary visibility fact with a three-valued publication status (`draft` editable · `private` and `public` both published and both **frozen**, public the default) and ruled that **unpublish returns an itinerary to draft**, the only way back to editing. That supersedes decisions 1, 2, 5 and 12 of this spec — including the live-projection premise the whole projection was designed around. **It is deliberately not implemented here:** it is a wire break, it is story-sized, and one question blocks it (the new `draft` collides with `ItineraryState.DRAFT` on the same object). Everything above stands as what S4.1 shipped; ADR-018 in `adr-log.md` and the epic-map backlog line carry what comes next.
