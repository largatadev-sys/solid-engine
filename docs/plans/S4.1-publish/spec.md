...
9. **The trip workspace gains an "Edit itinerary" cogwheel, opening the day editor.** Founder's call, and it completes the draft landing: a draft row opens the workspace, and the workspace is where you decide what to do next — publish, invite, or go build the plan. Shown only while the plan is editable (`canEditPlan`, i.e. not archived); when ADR-018 lands that becomes "only while draft", which is the one line that will need changing.
   **Wart, deliberately left rather than silently fixed:** the *day editor* already carries a cogwheel of its own, pointing back at the workspace's Details tab (S4.9's chrome, where the gear replaced the word "Details"). So the same icon now means two opposite things on two adjacent screens — "go to the editor" here, "go to trip settings" there. Not fixed because the answer is a design call, not a code one: either the workspace's affordance becomes a pencil, or the editor's gear goes and its back button carries the return trip.
