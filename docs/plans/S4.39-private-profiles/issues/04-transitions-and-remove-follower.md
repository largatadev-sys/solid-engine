...
- [x] **After removal** the removed traveler's relation is `none`, and they may follow (public owner) or request (private owner) again. ~~and no pending request is created or touched by the removal itself~~ — **amended during the build, 2026-09-04:** removal now also **cancels** a pending request. The original wording assumed removal only ever meets an existing follower, but a private owner can hold an edge and, after a later re-request, a pending row as well; leaving that row standing made the removed traveler read `requested` for ever, which is exactly the state decision 7 exists to prevent ("the removed traveler's relation becomes `none`"). Removal still *creates* nothing. Covered by `removingAlsoCancelsAPendingRequest_soRemovalAlwaysMeansNone`, sabotage-checked.
