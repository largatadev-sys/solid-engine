...
- [x] **An empty hidden set takes the existing query path unchanged**, asserted on both the first page and the after-cursor page for a viewer whose world has no private authors (`anEmptyHiddenSetWalksTheUnfilteredQueryOnBothPages`). **Narrowed during the build, 2026-09-04:** asserted as page shape and cursor behaviour rather than a literal byte comparison against a pre-change response, which nothing can capture once the change is in the tree. **The after-cursor half was genuinely missing until the code review found it** — the unfiltered `findFeedPageAfter` branch, which is the default every viewer takes today, had no test at all.
