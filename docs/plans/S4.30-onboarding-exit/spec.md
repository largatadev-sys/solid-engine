...
8. As a traveler resuming, I want the step number to reflect where I actually am, so that landing on "Step 3 of 4" is not the first evidence that step 2 was skipped past me.
...
4. **`nextOnboardingStep` gains `goals` and the step number stops lying.** The reason goals is absent is real and must be preserved: an empty `goals` array cannot distinguish "not asked" from "asked, chose none". The cheap honest fix is to resume at the first unanswered step among the three that *are* distinguishable and let the flow run forward from there — which means a resumed traveler starting at goals reaches interests and travel-setup as usual. Whether goals earns a distinguishable "answered nothing" state is a decision for the tickets; the alternative is dropping goals from the resume order permanently and saying so.
...
6. **The gate self-corrects out of onboarding.** `destinationFor` currently returns `null` for the `onboarding` segment in every branch. It must return the settled home when `nextOnboardingStep` is `null` — i.e. a traveler standing on an onboarding screen whose profile says they are done gets moved on, rather than being stranded by whatever put them there.
...
- **D1 — does skipping claim the suggested handle, or leave it null?** *Recommendation: claim it.* `suggestedHandle` is already computed, already prefilled on the profile step, and already the value a traveler gets by tapping Continue without editing — so claiming it on skip hands them the same handle by a shorter path. A null handle means no address other travelers can use and a fallback display name in every roster. **The invalidating condition is if the handle is meant to be a deliberate choice** — if it is, skip must leave it null, and the profile step becomes the one step that cannot be skipped, which is a materially different story.
