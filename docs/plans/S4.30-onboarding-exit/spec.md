...
> **SUPERSEDED IN ITS DIAGNOSIS, 2026-08-23 — read this before the rest of the section.** The reported defect was real and is now **fixed**, but not by anything in this spec. **The gate never redirected anyone.** Instrumenting `destinationFor`'s inputs at every render showed `destination = null` on *every* evaluation while the URL moved underneath it. The actual cause was in the root layout: `if (destination !== null || isSettling(gate)) return <Splash />;` returned the splash **instead of** the `<Stack>`, unmounting the navigator. On sign-in that window is always non-empty (`useMe` is loading), so **every** sign-in tore the navigator down, and expo-router's remount landed on `/onboarding/goals` — whereupon the gate applied its own correct rule (*never interrupt a traveler already walking the flow*) and left them there permanently, on an account whose `/v1/me` returned `onboardingCompleted: **true**`. Fixed by rendering the `<Stack>` on every path and overlaying the splash above it; guarded by `navigatorStaysMounted.test.ts` and recorded as a CLAUDE.md Gotchas entry.
>
> **What that means for the five consequences below.** Consequences **1–3** are real and remain unbuilt — they were never the reported symptom, and the tickets for them stand. Consequence **4** (the global `me` cache key) is real, survives this fix untouched, and is the most valuable thing left here. Consequence **5** is moot *as stated*: the navigator remount was the only observed source of a wrong entry, and decision 6 already ruled the ejection rule out on Edit-Profile grounds — but it remains the *amplifier* that would make consequence 4 permanent if 4 ever fires.
>
> **The closing note at the bottom of this spec — "a precise symptom is evidence" — reads as vindicated and is not.** The step number pinned a code path that turned out to be irrelevant, and that confident reading cost three rounds. The corrected lesson is in CLAUDE.md: *when a screen is wrong but every rule that could produce it is right, instrument the decision rather than re-reading the code, and suspect a **mount boundary**, not a routing rule.*
