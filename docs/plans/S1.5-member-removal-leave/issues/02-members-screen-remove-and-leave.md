...
- [ ] Owner: Remove on every non-self row, behind the confirm; roster updates on success
- [ ] Member: Leave trip behind the confirm; success lands on My Trips with the trip gone from the list and cache
- [ ] Owner sees no Leave control; member sees no Remove on others (Jest pins the gating)
- [ ] Both dialogs fire on native *and* web (the web fork is real, not the silent no-op)
- [ ] Missing-state copy reads sensibly for an evicted member (spec §5 wording check)
