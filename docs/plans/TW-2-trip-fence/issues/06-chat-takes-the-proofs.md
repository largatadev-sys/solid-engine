...
- [x] `send` takes a proof minted with chat's own refusal; the thread read takes `InAudience`; no method in the chat module takes a bare `Membership`
- [x] The chat module imports neither `WriteFence`, `AudienceFence` nor `PublicationState`
- [x] A member's send on a published trip answers `CHAT_CLOSED` with the same message; on an archived trip `ITINERARY_NOT_FOUND`; on a live trip the message lands — the chat ITs pass **unedited**
- [x] Scoped `chat` ITs and the unit suite green; CI green on push
