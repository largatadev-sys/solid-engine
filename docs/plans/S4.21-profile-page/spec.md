...
**Two deviations found at build time, beyond the six recorded above** *(agent, 2026-08-11, ticket 04)* — both forced by decisions the spec itself locks, neither a silent choice:

7. **The diary section header renders title + entry count, not the mock's "El Nido, Palawan · 5 days" with a cover thumb.** `DiaryTripResponse` is `{itineraryId, title, entryCount}` — the wire carries no destinations, no day count, no cover — and decision 7 ships the existing listing *unchanged* while the wire-changes section rules out touching the diary wire. Enriching it client-side by joining against the trips listing would be wrong by construction: a diary trip the traveler has since left is not in their memberships, so the join would blank exactly the sections that most need a label. The thumb ships as the mock's coloured well with no image. **Whoever widens `DiaryTripResponse` (additively) gets the mock's sub-line for free.**

8. **The profile's postcard is a NEW component (`src/diary/Postcard.tsx`), not an extraction of the diary stream's.** Decision 10 says "reused, not forked", and ticket 04 says the diary surface's own rendering is unchanged with its tests still passing — but there was no postcard *component* to reuse: the diary stream renders inline, on a different anatomy (eyebrow above the title, a 280px photo strip, divider-separated rows) from the mock's (badge beside the title, a full-width paged carousel with counter and dots, a bordered card). S3.1's mock covered the capture flow only, so **this mock is the first design baseline a rendered postcard has ever had**. Refactoring the stream onto a shared component would need three variant axes for no visible gain and real regression risk on a founder-visible surface. What is genuinely shared is the pure layer — `postcardAnatomy` plus the new `postcardCarousel` helpers — which is this repo's anti-drift mechanism anyway. **Backlog: the diary stream adopts `Postcard` the next time it is touched**, at which point one anatomy wins and the fork closes.
