...
**The design baseline landed 2026-08-20**: `Trips Spec.dc.html` (six frames, component contracts C1–C6, motion contracts M1–M4, a live demo) + `support.js` (the Claude Design runtime it renders with) + the handoff README — **founder to drop the three exported files beside this digest** (the conversation transfer mangles their UTF-8, so the originals are archived, not a retyping). Under the mock-fidelity rule the canvas is **normative**: its annotations are rulings, its copy strings are exact. This digest records what it settles, what it amends, and the one conflict it opens.
