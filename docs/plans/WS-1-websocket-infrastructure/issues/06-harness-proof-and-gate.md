...
This rung covers what the deployed spike structurally could not: the browser handshake puts **`Origin: http://localhost:8081` on the wire**, where the Node spike sent none — so both the origin-allowlist path and the header-less native path are exercised. **It does not, however, complete AC 1**: this runs against `localhost`, so browser-through-the-**edge** remains untested. Node+edge and browser+local do not compose into browser+edge; ticket 01 carries that note.
...
**2026-08-21, review pass — three defects in this ticket's own spec, found by review and fixed.** (1) The spec bypassed `IDENTITY_MAP`, so `unmetExclusivity()` could not see it and `fullyParallel` silently shared t1/t2 with `web/ownership-transfer`, which declares `wants-exclusive` — **the exclusivity check reported clean while being violated**. Registered as `web/socket-echo`. (2) The **Firebase ID token was passed into `page.evaluate`**, and `trace: 'retain-on-failure'` would have written it into `e2e/.artifacts/` on every failure — P3 extended from logs to durable artifacts. The ticket is now minted in Node and only the **single-use** value crosses into the page. (3) `requireStack` gated on t1 only, so a rejected **t2** token would have **failed** the spec rather than skipping it — defeating the gate's whole purpose of making *"never ran"* distinguishable from *"product broken"*. Both tags are gated now. Also: the unknown-action test asserted only that the connection survived, never that the error frame arrived; it now asserts `UNKNOWN_ACTION` explicitly.
