...
- *2026-08-30, at the gate:* two letter-breaches of "Old files never change", both deliberate, disclosed and flagged for founder review at the PR — (1) `media/PhotoSubject.java` gained the additive `POSTCARD` constant (media is a keeper the boundary guard permits, and the constant is unreachable through every existing flow); (2) one assertion in the old world's `DiaryContractIT` was re-pinned, because it pinned the entry→activity SET NULL that this spec's own signed V52 drop removes — the spec's "existing suites pass without a single edited test" and its FK drops were in unnoticed contradiction at exactly that line, and CI's full run found it. The rest of the old world is byte-identical, diff-checked.
- *2026-09-05, at landing (the rot-fix, no grilling):* the branch merged five days after S4.39 and PL-2 moved underneath it, so five things changed on the way in and none of them is a model change — migrations renumbered V49–V52 past `dev`'s V48; the ADR became ADR-035 because S4.39 took 034; the itinerary object **lost its public-or-private audience** and publish takes no body, since visibility is the profile's now; every new-world read (publication by its recorded owner, diary and postcard by their author, postcard photos through the media seam) is fenced by `AuthoredContentAudience`, replacing the interim public-at-posting posture this spec recorded; and the trip-derived postcard snapshot carries PL-2's pin. **The Diary this spec describes (title-only album) is re-cut at CM-2** — a collection with title, destination, cover, dates and a Day level, grilled 2026-09-05 (`docs/plans/CM-2-diaries-and-postcards/grilling.md`); read that record before wiring anything to the dark diary grammar.
