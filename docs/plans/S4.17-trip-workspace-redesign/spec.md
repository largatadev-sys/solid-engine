...
**2026-08-09 — decision 4's per-action saving is REVERSED by a later founder ruling; S4.18 will buffer edits until Save Changes.** Decision 4 made *Save Changes* the session's end and kept every edit saving per action (*"it persists nothing, because every edit already saves per action"*). On the running build the founder reordered activities, pressed back without saving, and found the change already persisted — **the label promises a save that has already happened, so leaving without pressing it reads as discarding when it does not.** Two fixes were weighed openly: relabel to `Done` (cheap, honest, keeps per-action crash-safety) or buffer for real; **the founder chose buffering**, and it is pulled as **S4.18 with a grilling first**, because it reverses a ruled decision and adds a wire endpoint. **S4.17 ships as built and is unaffected** — this comment exists so the next reader of decision 4 learns it is superseded rather than trusting it. Scope, open questions (photos · discard-on-back · crash persistence) and the lost-update trade-off are on the epic map's S4.18 line.
...
**2026-08-09 — decision 6's tap-the-title day rename is SUPERSEDED by S4.19; decision 1's chip copy is amended there too.** The founder's workspace-smoothing grilling ruled the invisible tap-to-rename out: a visible pencil-square (the activity rows' icon) joins the expanded day header and becomes the *only* rename affordance — the input, its placeholder and its blur-commit are unchanged, but the title's own Pressable retires (*"remove the title tap. pencil will be the editing feature"*). The same session re-ruled the editor chip: `Draft TRIP Workspace` → **"Trip Workspace"**, held twice over the glossary flag. This comment exists so the next reader of decisions 1 and 6 learns both are amended rather than trusting them. Spec: `docs/plans/S4.19-workspace-editing-pass/spec.md`.

**2026-08-09 — decision 9's row-tap destination is SUPERSEDED by S4.20; the ticket-06 "route stays" ruling is narrowed, not reversed.** Tapping a Travelers-tab row will open a read-only co-traveler profile stub, not the member-management screen; the role text moves to an owner-only tag. The `/members/[itineraryId]` route still resolves — the ownership-offer banner keeps its deep link, which becomes the screen's only door — so remove/leave/ownership-transfer go dormant until the profiles story re-homes them (founder-consented gap, epic-map line). Spec: `docs/plans/S4.20-travelers-profile/spec.md`.
