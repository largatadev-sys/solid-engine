...
| **Upcoming** | The lifecycle state between planning and travel: planning is finished, the trip has not started (ADR-020). The creation flow's terminal act — **Finish Planning** — lands here; `upcoming → draft` is "reopen planning". Editable like every unpublished state. *(Added 2026-08-04, create-flow grilling.)* |
...
| **Finish Planning** | The creation flow's terminal act: `draft → upcoming`. Deliberately **not** "Complete" — one word must not name both *done planning* and *the trip happened* (ADR-018's naming argument, third application). A milestone, not a padlock: the plan stays editable. *(Added 2026-08-04.)* |
...
**Traveler (organizer):** sign up → create Itinerary (scratch or fork, born `draft`) → invite co-travelers → Workspace forms → collaborate (days + activities, decisions — plan editing is leased per subject: activity, day, or itinerary header; ADR-014 as amended 2026-07-31) → **Finish Planning** (`draft → upcoming`) → trip runs (**Start trip** → `ongoing`; diary + expenses accrete) → **Mark completed** → **publish to the feed — only from `completed`** *(ADR-019/020: the feed carries records of trips that happened; publishing freezes the plan)* → reviews.
...
| From → To | Trigger |
|---|---|
| draft → upcoming | **Finish Planning** — the creation flow's terminal act |
