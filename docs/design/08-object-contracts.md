...
| `/v1/trips/**` | **The Trip, in its own vocabulary** — the new world's grammar (CM-1, dark). *From CM-3 (ADR-037): the whole workspace's grammar — **49 twins of the old root's 62 mappings**, each declared on the same handler, beside the acts that were always here (destruction, publish, the trip-day postcards). Measured at the build; the spec's "47 of 60" was written from an earlier count and the excluded set is unchanged.* **The 13 with no twin:** the 8 old diary-entry routes (content, retired at CM-5) and the five acts of the old lifecycle — `fork`, `publish`, `unpublish`, `audience`, `finish-planning` — which answer at the old root only because a trip is never published and forking is an itinerary's act (ADR-037). `TripGrammarTwinIT` names any route that drifts from this. | CM-1 · CM-3 |
...
|---|---|---|---|---|
| Read the trip | `GET /v1/trips/{tripId}` | any member (archived: owner only — the audience-fence posture) | *Amended 2026-09-07 at CM-3:* the **full record the old detail answers** — thirty fields including the plan tree, `viewerRole` absent on the detail as on the old root. CM-1's dark ten-field read and its `TripResponse` retired with it: one handler now serves both roots, so there is one shape and it cannot drift. | non-member, or member of an archived trip: `404 ITINERARY_NOT_FOUND` — one handler answers one code, on both roots, until CM-5 |
...
| Act | Endpoint | Authority | Answer | Refusals |
|---|---|---|---|---|
| Publish = mint | `POST /v1/trips/{tripId}/publish` | owner; trip must be `completed` | `200` with the object: `{id, tripId, publishedAt, plan}` — `plan` is the frozen document (header facts + days + activities, textual). Also flips the **same trip record** the old world reads (`published`, `published_at` — `visibility` is a constant `public` shim since V48) — one truth, never forked. First publish mints; republish **refreshes the same object** — the id never changes across cycles (`UNIQUE(trip_id)` makes this structural). | member: `403 NOT_PERMITTED` · stranger: `404 TRIP_NOT_FOUND` · not completed: `409 ITINERARY_NOT_COMPLETE` · bad audience: `400 UNKNOWN_AUDIENCE` · **another member holds the editing session: `409 EDIT_LOCKED`** *(added at CM-3 — the old route always refused this and the dark route did not, so the two acts differed on an invariant; the message names the holder, as the old one does)* |
| Unpublish = retire | `POST /v1/trips/{tripId}/unpublish` | owner | `204`. The object retires (reads mask) **but keeps its id** — every link ever shared resurrects on republish. The trip record flips back. *Amended at CM-3: **a trip with no live object is not a refusal** — the flag is cleared and `204` answered either way, because a trip published by the old flag-flip has no object to retire and the traveler's act must still succeed.* | member: `403 NOT_PERMITTED` · stranger: `404 TRIP_NOT_FOUND` |
...
| Edit | `PATCH /v1/diaries/{diaryId}` body `{title, startDate, endDate, destination?, pin?}` | author | `200`. **Changing the dates never creates or deletes a day** — and can never strand one: dates that would leave an existing day outside them are refused. This is the only act that moves a diary start, so whenever the start moves, in either direction, every **dated** day is renumbered by the same delta - the gaps are preserved, and derived trip days keep the ordinal they snapshotted from their trip day. | non-author: `404 DIARY_NOT_FOUND` (masked) · a day would fall outside: `409 DIARY_RANGE_STRANDS_A_DAY` · the create's validation refusals, by the same names |
...
| Act | Endpoint | Authority | Answer | Refusals |
|---|---|---|---|---|
| Add a day | `POST /v1/diaries/{diaryId}/days` body `{date, place?, pin?}` | author | `201` the day. Its **ordinal is the date's distance from the diary's start, plus one** — so filling Mar 15, 16 and 19 of a Mar 15–19 diary reads **Day 1, Day 2, Day 5**: a skipped day leaves a gap rather than renumbering its neighbours. A date **outside the diary’s dates is refused**, not absorbed: the calendar greys those days out and the server refuses one anyway. Widen the diary first, then add the day. | no date: `400 DIARY_DAY_NEEDS_A_DATE` · outside the diary’s dates: `400 DIARY_DAY_OUTSIDE_RANGE` · a day already on that date: `409 DIARY_DAY_ALREADY_EXISTS` · place >200 chars: `400 DIARY_DAY_PLACE_TOO_LONG` · non-author: `404 DIARY_NOT_FOUND` (masked) · a pin with no name: `400 INVALID_PIN` · a pin off the earth or at a zoom the provider does not serve: `400 INVALID_PIN` |
...
*Added at CM-3:* `EDIT_LOCKED` on `POST /v1/trips/{id}/publish` — **not a new spelling**: the old route has always answered it, and the dark route was missing the guard, so this closes a difference between two acts rather than minting a refusal.
...
**Every row below reads at `/v1/trips/**` too, since CM-3** — the same handler, the same shapes — *except* the five lifecycle acts of the old controller (`publish`, `unpublish`, `audience`, `finish-planning`, `fork`) and the eight old diary-entry routes, which answer at the old root alone. The old root is served unchanged until CM-5.

| Surface | Endpoints | Vocabulary reading |
|---|---|---|
| **The Trip** (record + lifecycle) | `POST/GET /v1/itineraries` · `GET/PATCH /v1/itineraries/{id}` · `POST …/{id}/start` `…/complete` `…/reopen` `…/archive` `…/unarchive` · `POST …/{id}/finish-planning` *(dormant, permanently refusing — **and untwinned**)* | The trip's own facts and ladder. `viewerRole`/`memberCount` additive since S4.38. Twinned at CM-3 apart from `finish-planning`; `GET /v1/trips/{id}` is this row's detail, not CM-1's retired ten-field read. |
...
| **Publishing (old flow)** | `POST /v1/itineraries/{id}/publish` `…/unpublish` · `GET /v1/itineraries/{id}/preview` · `GET /v1/published-itineraries/{id}` · `POST /v1/itineraries/{id}/fork` | The projection-based published page (ADR-017/019). Runs unchanged through the dark window; the cutover backfill mints objects for currently-published trips, and the rewire decides these endpoints' fate. **Since CM-3 only `preview` is twinned**: publishing and forking are not trip acts (ADR-037), so they keep the old root alone. **The client still calls this flow's `publish`/`unpublish`, deliberately** — CM-3 moved it to the itinerary module's route and then moved it back before merge, on the founder's call, so that the story changes no behaviour a traveler can reach while `dev` is the only running environment (epic-map line 227: `preprod` and `prod` are deferred). The itinerary module's `POST /v1/trips/{id}/publish` stays dark and correct — both its editing-session guard and its tolerant unpublish shipped at CM-3 — and **CM-5 moves the client onto it with the readers**, which is when the object becomes the thing being read. |
...
| **Workspace world** | members/invitations (`/v1/itineraries/{id}/invitations[…]`, `/v1/invitations/{id}/accept|decline|revoke`, `DELETE …/members/{travelerId}`) · ownership offers · polls · chat · photo dump · join links (`/v1/itineraries/{id}/join-link`, `/v1/join/{token}[…]`, join requests) | Everything membership-authorized. All of it is what `DELETE /v1/trips/{id}` destroys. **Twinned whole at CM-3** — the trip-rooted routes in `poll`, `chat`, `join` and `invitation` each gained the second root on their existing mapping, one line per controller. The object-rooted routes (`/v1/invitations/{id}/…`, `/v1/join/{token}`) are not trip-rooted and are unaffected. |
