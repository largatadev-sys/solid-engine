...
| Edit | `PATCH /v1/diaries/{diaryId}` body `{title, startDate, endDate, destination?, pin?}` | author | `200`. **Changing the dates never creates or deletes a day** — and can never strand one: dates that would leave an existing day outside them are refused. This is the only act that moves a diary start, so when the start moves earlier every **dated** day is renumbered by the same delta - the gaps are preserved, and derived trip days keep the ordinal they snapshotted from their trip day. | non-author: `404 DIARY_NOT_FOUND` (masked) · a day would fall outside: `409 DIARY_RANGE_STRANDS_A_DAY` · the create's validation refusals, by the same names |
