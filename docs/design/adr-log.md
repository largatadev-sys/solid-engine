...
- **Open, and blocking a build.** Does `draft` become the `visibility` column's third value, or a differently-named field? The word collides with `ItineraryState.DRAFT`, the dormant lifecycle's first state, on the same object — two unrelated "draft"s is exactly the ambiguity the domain model exists to prevent.
