...
- **Diary is a first-class object** — ~~the shared-album model. One **author-owner** per diary; contributors join only by the owner's consent; publication is the owner's sole right.~~ **the postcard model since 2026-08-11 (ADR-024): one traveler's stream of activity-derived posts per trip — no contributors, only the author writes.** A diary **references one itinerary**; one itinerary (and its fork lineage) can have **many diaries** — ~~the owner's, collaborators', forkers'~~ **exactly one per traveler** — each an independent lived perspective on the same plan. Published diaries surface as **Highlights** on the itinerary *(the publish act builds at S4.2; until then diaries are author-only, on the author's profile)*. The group's shared thing is the **Photo Dump** (S3.4), not a co-authored diary.
...
- **Visibility (v1 enum):** ~~public / unlisted (link-only) / private~~ **`private / published` — binary since S4.1 (2026-07-31, ADR-017): unlisted deleted by founder ruling ("it's just private and published").** Friends-only remains a designed-for, deferred value — it arrives with the future friend-graph feature. **Superseded 2026-09-04 (S4.39, ADR-034): the itinerary carries no visibility value at all — `published` is its one exposure switch, readable by every signed-in traveler; public/private moved to the Traveler as Profile Visibility, and the friend-graph value is closed for good.**
