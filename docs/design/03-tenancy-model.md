...
|---|---|---|
| **World-readable** | Published itineraries, Highlights (published diaries on published itineraries), reviews, public comments, stars, public profiles, **aggregate trip cost only** | `published`: anyone *(visibility is **binary** since S4.1/ADR-017 — unlisted deleted, `friends_only` reserved for the friend graph; accountless reach itself arrives with the backlogged web read-only surface)* · writes always require an account (INV-3) · the published projection obeys **the absence rule** (INV-2: no dates, lifecycle state, stamps, roster, or attribution) *(Amended 2026-09-04, ADR-034: the itinerary has no visibility value — `published` alone, readable by every signed-in traveler. "Public profiles" here means the header, counts and published showcase; a private profile's postcards and lists move to the profile-walled row below.)* |
| **Profile-walled** (ADR-034) | A **private** traveler's postcards (Home, the profile's diary tab, the per-trip diary, the photo bytes) and their followers/following lists | The author, and their **approved followers** — the Follow edge is the authority; a stranger gets `403 PROFILE_PRIVATE` on lists and `404` on bytes. Membership in a shared trip grants nothing here. A public traveler's same objects are world-readable. |
