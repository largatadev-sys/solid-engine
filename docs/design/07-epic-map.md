...
**Skeleton done = the playbook's exit criterion met:** auth → API → guard → domain → Postgres → response, running on the prod target, **installable on a phone via sideload** *(amended 2026-07-16: the original "installable via store track" criterion travels with the parked Play story below — E0 completes without it, deliberately and on the record).*
...
- **Play internal testing track** — *(parked at S0.4, 2026-07-16; carries E0's original "installable via store" criterion)* **trigger: pre-alpha, before E6 completes** — the alpha ships through it (dozens of testers need store installs + silent auto-updates; sideload version-chaos poisons the alpha's signal). Contents when pulled: Play listing + data-safety forms + content rating · AAB upload (same Gradle build as the sideload APK, same keystore) · internal track pointed at preprod · mobile-side `largata-prod` wiring (`google-services.json`, providers, web client id). **Two recorded landmines:** (1) Play re-signs the AAB — **Google sign-in breaks on every Play-delivered build until the Play signing key's SHA-1 is registered in `largata-prod`** (invisible in all sideloaded builds; S0.2-class native-layer surprise); (2) if the account is personal (post-Nov-2023 rule): 12+ testers × 14 consecutive days of closed testing before the *production* track unlocks — an alpha-planning input, not an internal-track blocker.
