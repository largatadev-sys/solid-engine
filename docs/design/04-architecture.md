...
- **Status.** Accepted · 12/07/2026 · **amended 28/07/2026** (S1.8 grilling — the seam's ship moment moves from Epic 1 to register #14's decision; full amendment text in `adr-log.md`)
...
- **Status.** Accepted · 2026-07-23 (S1.3 grilling, founder-confirmed)
- **Context.** Artifact 02's original Itinerary Item carried a calendar date/time, making "Day 3" a projection of dates. The 07/18 create-and-publish mock plans by **ordinal days** — duration picked at create, day tabs, per-day titles ("Arrival & Sunsets"), empty planned days — and itinerary dates have been *optional* since S0.3 (nullable, and ADR-008 keeps them forever). Date-grouping therefore collapses for undated plans, has no home for a day title, and cannot represent an empty day. Forking (S4.7) also wants relative days: copied absolute dates are someone else's trip.
- **Decision.** The Itinerary aggregate gains a **Day** child (contiguous ordinal 1…N — deletion renumbers; optional title); **Activities** belong to a Day with a **manual sort order as the authoritative ordering** (optional time-of-day is display metadata — local, timezone-free — so a drag never snaps back against a typed time). `startDate`/`endDate` remain optional itinerary metadata; when present, Day N's calendar date is **derived** (start + N−1), never stored. **No date-span↔day-count invariant in MVP** — under last-write-wins collaboration it would couple two members' independent edits into mutual invalidation (one stretches the dates while the other deletes a day); the lifecycle story (S1.7) revisits when dates start meaning something. The entity noun is **Activity**, replacing "Itinerary Item" (founder call); `type`/`source` stay canon shape but defer as columns to their first reader (E6/E4).
- **Alternatives rejected.** Date-anchored items (the original shape) — breaks for undated itineraries, holds no day titles or empty days, copies meaningless dates on fork. Dropping dates entirely — ADR-008 forbids removing shipped /v1 fields, and register #10's lifecycle nudge wants them.
- **Consequences.** `/v1` speaks `days` and `activities` — **the wire noun is permanent within v1 (ADR-008), accepted knowingly**: E6's unfurled hotels and flights will live under `activities` even when the name reads narrow. Ordinal contiguity is the aggregate's internal consistency job (strong consistency within, per Artifact 02).
- **What would invalidate it.** Real usage demanding calendar-anchored planning (cross-timezone flight times, calendar sync) → add an optional per-activity datetime *additively* beside the day structure; the ordinal skeleton survives.

### ADR-014 — MVP concurrency: single-writer itinerary edit lock (supersedes S1.3's last-write-wins)
- **Status.** Accepted · 2026-07-24 (S1.4 grilling, founder-ruled) · **amended 2026-07-31** (trip-surfaces reconciliation, founder-ruled — the lease becomes subject-typed: activity / day / header; ships at S4.9) · partially supersedes ADR-001's queued-writes clause for plan content; waives ADR-008 for the S1.3 write endpoints (recorded below)
- **Context.** S1.3 shipped last-write-wins whole-entity writes with `last edited by/at` attribution — its AC 7 *pinned the absence* of any conflict surface, and the 07-17 UX reconciliation had parked locking post-gate as one of the two most expensive boxes in the flow set. Founder ruling 2026-07-24, made against a restatement of that analysis: silent overwrites are not acceptable for the MVP; while one member edits, the plan is closed to everyone else. The declared future is live/real-time collaborative editing — this lock is its deliberate stopgap.
- **Decision.** A **whole-itinerary lease lock**, one row per itinerary (holder traveler id + `expires_at`; an expired row counts as no lock). Entering *any* plan-edit surface (itinerary fields, day title, activity add/edit/reorder/delete) acquires the lease; a **TTL of ~3 minutes** is auto-renewed while the edit screen stays open; save/cancel releases immediately — but **expiry is the real guarantee** (a dead or backgrounded client never sends a release; the lease frees itself). **No force-take, the owner included.** Enforcement is **server-side on every plan-write endpoint** (guard resolves membership first, then the lease is checked; a caller without the lease is rejected) — the client modal ("*{name} is editing this itinerary*") is a *response* to a failed acquire, never a live broadcast: clients are pull-based, so lock state is only learned at the moment of attempting to edit.
- **Amendment (2026-07-31 — the trip-surfaces reconciliation, founder-ruled; ships at S4.9).** The lock's granularity moves from whole-itinerary to **subject-typed leases**: one lease table, subjects `header | day | activity`, each row keeping the original semantics per subject (holder + `expires_at`, ~3-min TTL auto-renewed, release on save/cancel, **expiry the real guarantee**, no force-take, server-enforced guard-then-lease on every plan write). This supersedes both the whole-itinerary scope and this ADR's own rejection of per-item locks — and the recorded objection to per-item, *"partial protection,"* is answered structurally rather than inherited:
  - the **activity lease** guards an activity's field edits **and its deletion**; the **day lease** guards the day's own fields (title) and its deletion — it does **not** imply the activities inside; the **header lease** guards the itinerary's fields (title / destinations / dates / description — the `edit.tsx` surface, flow unchanged);
  - **adds are unguarded** (day and activity): additions commute, nothing is overwritten. Interim permission ruling taken at the same session: **add/delete day is owner-only "for now"** — a narrowing of S1.3's members-edit-plan-content split on shipped endpoints, ADR-008 waiver renewed knowingly (clients still founders-only), revisit trigger = the validation gate;
  - **delete day** additionally rejects (409, naming the holder) while any contained activity is leased by another member — a day cannot be yanked out from under an active editor;
  - **reorder is version-checked, not leased**: the whole-list PUT carries the ordering it believes current; a stale set or stale order → 409 → refetch and re-apply. With this, **no write in the model can silently overwrite another** — the principle this ADR was ruled on, now held at finer grain;
  - **the lease gains a read surface** (additive fields in plan read payloads: holder identity per subject): clients stay pull-based; the "*Being edited by @…*" indicator is **advisory** — it may be a refetch-interval stale, it never disables an edit attempt, and the failed-acquire modal remains the enforcement surface. This is not presence: no push, no sockets, no broadcast.
  - Unchanged by the amendment: offline plan-editing stays disabled (ADR-001 partial supersession stands), expiry self-healing, no force-take, and live editing as the declared replacement — the leases are still built to be discarded.
- **Alternatives rejected.** Keeping LWW — shipped and cheapest, rejected by founder ruling on the merits after challenge. Per-item locks — the 07-17 parked shape; whole-itinerary chosen deliberately: total protection, one lease row, no per-entity lock bookkeeping *(superseded by the 2026-07-31 amendment: the 07/31 mock set's collaboration surface made the serialization cost concrete, and the amendment answers the bookkeeping/partial-protection objections structurally — see above)*. Live editing now — presence + concurrent-sync infrastructure that validates nothing the alpha tests; it stays the post-gate target.
- **Consequences, each accepted knowingly.** (1) **Offline plan-editing is disabled** — a minutes-scale lease cannot be held across an hours-scale offline gap, so ADR-001's queued writes now *exclude plan content*; offline stays read-only for the plan (partial supersession of ADR-001). (2) **The shipped S1.3 /v1 write endpoints gain a rejection case** — a semantics change within /v1 that ADR-008 forbids; **waived knowingly by the founder** (2026-07-24) while the only installed clients are the founders' own, recorded here and in the S1.4 spec. (3) **S1.3's AC-7 IT is deliberately replaced** — the new tests pin the lock surface instead of pinning its absence. (4) **Editing serializes per itinerary** — one writer at a time is the accepted MVP posture for alpha group sizes; the busier the planning, the more the modal is seen.
- **What would invalidate it.** The validation gate showing serialization hurting real collaboration, or live editing arriving post-gate → the live-editing backlog line supersedes this design wholesale; the lease was built to be discarded.

### ADR-015 — Traveler handle (@username): a unique, changeable label; the id stays the identifier
- **Status.** Accepted · 2026-07-30 (S4.0 grilling, founder-ruled) · supersedes the 2026-07-17 "no handles in the MVP" ruling; closes the epic-map handle decision ahead of its pre-S4.3 pin
- **Context.** S0.2 ruled the display name non-unique and *never an identifier or lookup key*; the 07-17 UX reconciliation kept handles out of the MVP and registered the decision for pre-S4.3, because E4's discovery cards are the first surface that renders one. The S4.0 auth/onboarding wireframes reintroduced the field, and the founder pulled the decision forward: the full onboarding flow ships at S4.0 and collecting the handle there beats retrofitting a prompt before S4.3.
- **Decision.** Traveler gains a **handle**: 3–20 characters, `a–z 0–9 _`, stored lowercase, **globally unique case-insensitively** (DB unique constraint), required at onboarding with a collision-free prefilled suggestion, **freely changeable**, released immediately on change, guarded by a small reserved-word blocklist (`largata`, `admin`, `support`, `api`, …). The load-bearing rule: **the handle is a unique label, never a key** — every API route, foreign key, and any future profile URL stays id-keyed, which is what makes free change + immediate release safe (a stale handle in an old shared link resolves to nobody, not to a squatter). Availability check is its own additive endpoint. Existing accounts carry NULL until they complete the S4.0 profile step — client-routed, never backend-enforced.
- **Alternatives rejected.** No handles until S4.3 — the registered plan, superseded by the full-onboarding pull. Immutable or tombstoned handles — protects against a takeover vector the id-keyed rule already eliminates, at a standing support cost.
- **Consequences.** Display name stays the non-unique human label (S0.2 unchanged in spirit: nothing keys on a name). Uniqueness semantics get a pinning test (the V4 lesson generalized: any constraint whose meaning can silently move needs a test that fails when it does).
- **Amendment, 2026-07-31 (S4.9, founder-waived): a handle is an invite address, and `invitation.email` becomes nullable on the wire — an ADR-008 semantics change, waived knowingly.** S4.9 added invite-by-handle beside invite-by-email. A handle-addressed invitation has **no email** — deliberately: the handle path must never look up an address, because resolving one would turn a username into a way to harvest email, and the DB `CHECK ((email IS NULL) <> (invitee_traveler_id IS NULL))` makes the two modes mutually exclusive rather than merely conventional. The consequence reaches `/v1`: `email` on `GET /v1/itineraries/{id}/invitations` was always present and is now **sometimes null**, with `inviteeTravelerId` / `inviteeHandle` carrying the address instead. A field going from always-present to sometimes-null is a **semantics change ADR-008 forbids**, and it is **waived by the founder (2026-07-31)** on the same grounds as ADR-014's waiver: the only installed clients are the founders' own, and no released version reads that field. **Rejected alternatives, both worse:** a parallel handle-invite endpoint (forks every lifecycle operation — list, resend, revoke, accept — and drops the single-pending guarantee to application code, since a partial unique index cannot span two tables); and synthesising an email for handle invites (invents data, and re-introduces exactly the address lookup the handle path exists to avoid). The single-pending rule needed a **second** partial index rather than one, because SQL NULLs never collide — so `(itinerary_id, email)` cannot constrain handle rows.
- **What would invalidate it.** Real impersonation or abuse via handle churn → add a change cooldown/tombstone additively. Vanity profile URLs wanting handles → that story adds redirect-on-change semantics; the id-keyed rule stands.

### ADR-016 — Brand palette & type: terracotta / navy / cream + Inter, adopted globally as token values
- **Status.** Accepted · 2026-07-30 (S4.0 grilling, founder-ruled) · discharges the "visual direction & design tokens" decision ahead of its pre-S4.3 pin
- **Context.** S0.3 built the token layer (semantic colour roles, type scale; hardcoded values banned) with explicitly-interim values, deferring the brand call with the rule *"a decision, not a side effect."* Two mocks then disagreed: the 07/18 trip mock (orange `#FF751F`, zinc neutrals) and the 07/30 auth/onboarding wireframes (terracotta, navy, cream, Inter). Building the onboarding screens forced the choice: one palette globally, or a two-toned product.
- **Decision.** The onboarding wireframes' palette is the brand: accent **terracotta `#D96C4A`** · ink **navy `#1B263B`** · background **cream `#FAF9F6`** · secondary text `#5C6470` · border `#E2E4E8` · white surfaces, with **Inter** as the type family. Applied as a **values-only swap** in the theme tokens; screens keep consuming tokens only. Supersedes the 07/18 mock's orange as brand evidence.
- **Alternatives rejected.** Interim tokens until S4.3 — makes the decision twice and ships onboarding unlike its own design. An onboarding-only palette — a two-toned product and a standing invitation to hardcode values.
- **Consequences.** Every existing screen re-skins in one commit with zero structural change — the S0.3 token bet paying out. Web and mobile stay identical per the standing parity principle. Inter loads as an app font on native and web (an asset, not a system default).
- **What would invalidate it.** A professional brand pass before public launch → same mechanism, new values; the tokens are the seam.

### ADR-017 — Publish: a binary visibility flip with a live rule-scrubbed projection; the audience ladder
- **Status.** Accepted · 2026-07-31 (S4.1 grilling, founder-ruled) · resolves register #11 whole · re-rules S1.9's archive *sight* ("evicts nobody" → owner-only) and re-semanticizes S1.3's `notes` (**Creator Tips** — public on publish, copied on fork); ADR-008 waivers renewed for both, clients founders-only · retires `completed` as a gate (the 2026-07-29 dormant-states ruling's assigned question)
- **Context.** Register #11 had accumulated: UX flow 12's snapshot-publish proposal (a scrubbed public copy), publish-from-creation proposed twice (flow 6 + the 07/18 mock's create → preview → publish → success arc), the state machine's `completed → published` transition, the est-vs-actual cost question, and — sharpest, 2026-07-29 — the safety constraint that a published itinerary must not reveal whether its traveler is currently away from home, binding the whole projection (state, dates, diary recency, anything derivable).
- **Decision.** Publishing flips a **binary visibility fact** (`private → published`) on the one itinerary — a second fact **orthogonal to the dormant lifecycle** (one enum cannot hold "completed *and* published", the only kind of itinerary a review can land on). The public surface is a **live projection scrubbed by rule** (INV-2 as amended), never a copy. **No lifecycle gate** — publish from creation onward, owner-only, with a **symmetric unpublish** (social objects hidden-not-deleted; forks survive; not-found masking indistinguishable from never-published). **The absence rule:** the projection never carries absolute dates, lifecycle state, or lifecycle stamps — duration derives from the day count; the date span never crosses. **The audience ladder:** archived = owner only · private = owner + collaborators · published = everyone; archive dominates publish (instant take-down), unarchive restores the prior audience, public included. **Unlisted is deleted; `friends_only` is reserved** as a future additive value awaiting the friend graph.
- **Alternatives rejected.** Snapshot-publish — protects member data by construction, but INV-2 already does so by rule; it kills the live-derived aggregate cost (canon's own recorded objection), mints a second identity for stars/reviews/forks to attach to, and makes publish the first side-effectful transition (the S1.7 race-test line would fire). Two modes — doubles the semantics to explain for no consumer. A `completed` gate — resurrects the Start/Complete UI removed at the E1 gate, cannot classify a dateless trip (trips are born dateless since S4.9), and buys safety the projection shape already provides. Absolute dates as opt-in — a consent surface spent exposing a fact with no consumer (a forker needs duration and season, never someone else's calendar).
- **Consequences.** Publish stays side-effect-free — S1.7's race-test exemption holds. Post-publish edits are instantly public; a review may describe a plan that has since changed (recorded as S4.5's question). Members lose sight of archived trips (self-leave stays permitted on the wire, unreachable from UI while hidden — accepted). Empty publishes are possible, accepted knowingly. The dormant lifecycle chain ships untouched (ADR-008); `completed`'s value survives only as E4's social-proof signal.
- **What would invalidate it.** Real-world trust abuse of live-updating published plans → freeze/versioning revisited additively at that evidence. The friend graph arriving → `friends_only` slots in as a third value. Accountless reach (the web read-only surface) → the same projection serves it; the rule-scrubbed field set is the contract.

---

**Resolution: ☑ Agreed** *(proposed solo — pending founder ratification; ADR log accretes from here — every future significant decision lands as ADR-018+)*
