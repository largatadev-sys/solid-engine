...
| S4.16 | The cover and the trip — making creation one act instead of four *(raised by the founder at S4.15's fix pass, 2026-08-08, then sharpened the same day: the complaint is the **ordering**, not the wait. **Needs grilling, not ready to build.** Creating a trip with a cover costs four sequential calls and the header lease is enforced server-side, so no client change can reorder it. The spec offers three shapes — born-together at create, staged upload-at-pick, and a lease exemption named only to be rejected — and question 0 picks one; born-together answers the complaint while killing the orphan, sweep and quota problems outright)* | ⬜ | [spec](docs/plans/S4.16-upload-on-pick/spec.md) |
