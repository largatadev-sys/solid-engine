...
| S0.2 | Auth end-to-end (Firebase → resource server → Traveler) | 🔄 | [spec](docs/plans/S0.2-auth-end-to-end/spec.md) · on `feature/S0.2-auth-end-to-end` (`git log --grep S0.2`) — **awaiting owner: merge approval + the one open AC**. Slice proven on the AVD against real `largata-dev`: email sign-up → Traveler provisioned once (UUIDv7, local-part display name) → `/me` renders it; sign-out/in creates no duplicate; `userId` in logs, no PII. 85 tests green (44 backend, 41 mobile); compose gate passed from scratch. Expo Go abandoned permanently — dev-build replaces it, which also closed **S0.1 ticket 05's two carried ACs** on-device. **Open: Google sign-in untested** (AVD has no Google account — needs the owner, ~3 min: ticket 04's Comments) |
