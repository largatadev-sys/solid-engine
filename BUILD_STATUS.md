...
| S0.2 | Auth end-to-end (Firebase → resource server → Traveler) | 🔄 | [spec](docs/plans/S0.2-auth-end-to-end/spec.md) · on `feature/S0.2-auth-end-to-end` (`git log --grep S0.2`) — **every AC met; awaiting owner's merge approval only**. Both providers proven on the AVD against real `largata-dev`: Google → `display_name: Largata` (name claim) and email → `ana.silva` (local-part) — both branches of the rule, two travelers, two UIDs. Provisioning idempotent across reloads, sign-out/in cycles and an account switch; session survives a force-stop; `userId` in logs, no PII. 85 tests green (44 backend, 41 mobile); compose gate passed from scratch. Expo Go abandoned permanently — the dev-build also closed **S0.1 ticket 05's two carried ACs** on-device. Only unverified line: the password-reset email (no real inbox) |
