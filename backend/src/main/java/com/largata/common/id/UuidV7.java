 ...
 *
 * <p><strong>Why app-side and not {@code gen_random_uuid()} DB-side.</strong> The id exists before
 * persistence, which module boundaries rely on (a module hands another an id without a round-trip),
 * and the DB version stays unconstraining — Postgres only grew a native v7 function in 18, and the
 * prod major is undecided until S0.4.
 *
