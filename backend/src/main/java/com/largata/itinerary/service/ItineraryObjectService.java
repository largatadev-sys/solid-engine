...
import com.largata.itinerary.exception.ItineraryObjectNotFoundException;
...
@Service
public class ItineraryObjectService implements ItineraryApi {
    ...
    public ItineraryObject read(UUID objectId) {
        return objects.findById(objectId)
                .filter(candidate -> !candidate.isRetired())
                .orElseThrow(ItineraryObjectNotFoundException::new);
    }
