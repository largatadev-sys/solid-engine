    ...
    public List<TrendingDestinationResponse> trending() {
        return itineraries
                .findTrendingDestinations(
                        Instant.now().minus(TRENDING_WINDOW), surface.archivedArray(), TRENDING_CAP)
                .stream()
                .map(
                        row ->
    ...
    public DiscoverySuggestionsResponse suggestions(DiscoveryFilters filters, UUID callerId) {
        ...
        return new DiscoverySuggestionsResponse(
                itineraries.suggestDestinations(
                        surface.archivedArray(), filters.query(), SUGGESTIONS_PER_GROUP),
                itineraries.suggestTitles(surface.archivedArray(), filters.query(), SUGGESTIONS_PER_GROUP),
