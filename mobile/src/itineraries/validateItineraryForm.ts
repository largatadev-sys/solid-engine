...
export function validateItineraryEdit(form: {
  ...
  if (form.description.trim().length > 4000) return 'A description may be at most 4000 characters.';
  if (form.startDate !== '' && !isCalendarDate(form.startDate)) return 'Start date must look like 2027-01-10.';
  if (form.endDate !== '' && !isCalendarDate(form.endDate)) return 'End date must look like 2027-01-10.';
