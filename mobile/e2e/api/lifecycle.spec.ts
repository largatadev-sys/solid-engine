...
test('a member completing an upcoming trip gets 403, not 409 — authority is checked before state', async () => {
  const refused = await api(`/v1/trips/${trip.id}/complete`, 'POST', member);
  expect(refused.status).toBe(403);
  expect(refused.body?.code).toBe('NOT_PERMITTED');
});

test('a member cannot start the trip, and nothing moves', async () => {
  const refused = await api(`/v1/trips/${trip.id}/start`, 'POST', member);
  expect(refused.status).toBe(403);
  expect(refused.body?.code).toBe('NOT_PERMITTED');
  expect(await stateOf(owner, trip.id)).toBe('upcoming');
});
...
test('startedAt and completedAt are deliberately not on the wire — no reader exists yet', async () => {
  const read = await api(`/v1/trips/${trip.id}`, 'GET', owner);
...
test('starting an already-ongoing trip is refused', async () => {
  const again = await api(`/v1/trips/${trip.id}/start`, 'POST', owner);
...
test('the owner completes the trip', async () => {
  const done = await api(`/v1/trips/${trip.id}/complete`, 'POST', owner);
  expect(done.status).toBe(200);
  expect(done.body?.state).toBe('completed');
});

test('the machine is forward-only — no re-complete, no way back by starting', async () => {
  const recomplete = await api(`/v1/trips/${trip.id}/complete`, 'POST', owner);
  expect(recomplete.status).toBe(409);
  const backwards = await api(`/v1/trips/${trip.id}/start`, 'POST', owner);
...
test('reopen walks the ladder down a rung at a time and floors at upcoming — no UI drives it', async () => {
  expect((await api(`/v1/trips/${trip.id}/reopen`, 'POST', owner)).body?.state).toBe('ongoing');
  expect((await api(`/v1/trips/${trip.id}/reopen`, 'POST', owner)).body?.state).toBe('upcoming');

  const nothingBelow = await api(`/v1/trips/${trip.id}/reopen`, 'POST', owner);
...
test('the trip walks the whole ladder again after the undo', async () => {
  await api(`/v1/trips/${trip.id}/start`, 'POST', owner);
  await api(`/v1/trips/${trip.id}/complete`, 'POST', owner);
...
test('completed is a working afterlife, not a freeze — the owner still edits the details', async () => {
  const lock = await api(`/v1/trips/${trip.id}/edit-lock`, 'POST', owner);
  expect(lock.status).toBe(200);

  const edited = await api(`/v1/trips/${trip.id}`, 'PATCH', owner, {
    title: 'edited after completion',
    destination: 'Palawan',
  });
  expect(edited.status).toBe(200);

  await api(`/v1/trips/${trip.id}/edit-lock`, 'DELETE', owner);
});

test('…while a member is refused the DETAILS, with the owner-act code (S4.25 decision 4c)', async () => {
  await api(`/v1/trips/${trip.id}/edit-lock`, 'POST', member);

  const refused = await api(`/v1/trips/${trip.id}`, 'PATCH', member, {
  ...
  await api(`/v1/trips/${trip.id}/edit-lock`, 'DELETE', member);
});

test('membership ops are not frozen either — the owner can still invite', async () => {
  const invited = await api(`/v1/trips/${trip.id}/invitations/by-handle`, 'POST', owner, {
...
test('PATCH cannot move lifecycle state — the two doors stay separate', async () => {
  ...
  await api(`/v1/trips/${trip.id}/edit-lock`, 'POST', owner);
  await api(`/v1/trips/${trip.id}`, 'PATCH', owner, {
...
test('the trip appears in My Trips carrying its state — what the row badge renders', async () => {
  const mine = await api('/v1/trips', 'GET', owner);
...
test.describe('S4.24 — the editor opens in every unpublished state, and the state survives the save', () => {
  ...
  const editsInPlaceAt = async (expected: string) => {
    const held = await api(`/v1/trips/${inPlace}/edit-lock`, 'POST', owner, {
      subjectType: 'session',
    });
    expect(held.status, `the Editing Session must open on a ${expected} trip`).toBe(200);

    const base = (await api(`/v1/trips/${inPlace}`, 'GET', owner)).body.planVersion;
    const saved = await api(`/v1/trips/${inPlace}/plan`, 'PUT', owner, {
      basePlanVersion: base,
      days: [{ id: inPlaceDay, title: `Edited while ${expected}`, activities: [] }],
    });
    expect(saved.status).toBe(200);

    await api(`/v1/trips/${inPlace}/edit-lock`, 'DELETE', owner, { subjectType: 'session' });
  ...
  test('an ongoing trip edits in place, and the diary blackout is gone', async () => {
    await api(`/v1/trips/${inPlace}/start`, 'POST', owner);
    await editsInPlaceAt('ongoing');

    const stillCapturing = await api(`/v1/trips/${inPlace}`, 'GET', owner);
    expect(
  ...
  test('an unpublished completed trip edits in place', async () => {
    await api(`/v1/trips/${inPlace}/complete`, 'POST', owner);
