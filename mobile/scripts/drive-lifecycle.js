/**
 * Drives the S1.7 lifecycle transitions in the **web preview container**, with the confirm dialog
 * intercepted (S1.7, ticket 03 / spec AC 8).
 *
 ...
 *
 * It also proves the two things unique to this story's UI, neither visible to Jest:
 *
 * - **the owner sees a lever and a member does not** — the same screen, two accounts, opposite
 *   renderings, which is the whole of spec decision 8's gating; and
 * - **an overdue draft offers Start, never Complete** — the strict two-tap path (decision 9) rendered
 *   rather than merely unit-tested, on a trip whose dates are both in the past.
 *
 * ## Usage
 *
 *   cd mobile && set -a && . ./.env && set +a
 *   node scripts/drive-lifecycle.js          # seeds its own trips via the API, then drives
 *
 * Assumes the preview container on :8081 and the backend on :8080 (the pinned port map in CLAUDE.md).
 * t1 = trip owner, t2 = ordinary member.
 */
...
const PORT = 9224; // not 9222 (drive-preview) or 9223 (drive-ownership-transfer)
...
(async () => {
  ...
  // --- seed two trips: one with future dates, one overdue ---------------------------------------
  ...
  // Both dates in the past: this is the forgetful owner's trip, and it must offer Start (not Complete).
  ...
  // --- 1. the owner's trip screen renders badge + lever ------------------------------------------
  ...
  // --- 4. complete, driven the same double way ---------------------------------------------------
  ...
  // --- 5. the overdue draft offers Start, never Complete (decision 9) ----------------------------
  ...
  // --- 6. a member sees the badge but no lever (decision 8) --------------------------------------
